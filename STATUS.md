...
> ⑤ **⚠️ B4 代码当前已从 main 临时 revert(bbeec50/83ba54b)**:推送后才意识到 Render Auto-Deploy=On 会直接上生产,而 **Supabase 生产库还没有 `users.stripe_sessions` 列**(生产 Payload 不自动推 schema,users 一查就炸 → 登录/admin 全挂);生产迁移动作(ALTER)需用户授权,故先 revert 保生产。**B4 代码原样在历史里(64f84ee 支付 + eda3443 免责),恢复 = 先给生产库补列,再 `git revert bbeec50 83ba54b && git push`**。补列 SQL(与本地 payload push 产物一致,幂等):`ALTER TABLE users ADD COLUMN IF NOT EXISTS stripe_sessions jsonb;`(Supabase Dashboard SQL editor 跑,或授权本机 psql 走 Session pooler)。
> **B4 收口待办**:① 上述补列+恢复 revert;② Stripe Dashboard(test)建 Product「Pro」+ 30/90 天两个 one-time Price、Payment methods 开 card+alipay、申请 wechat_pay → `STRIPE_SECRET_KEY/STRIPE_PRICE_30D/90D/STRIPE_WEBHOOK_SECRET` 填本地 `cms/.env` 与 Render env(⚠️ webhook endpoint 要同时订阅 `checkout.session.completed` + `checkout.session.async_payment_succeeded` 两事件,防 alipay 异步丢单)→ `stripe listen` + 4242/Alipay 模拟支付跑支付回归清单 → 勾 E3-03/04 §2 = **M2**。
>
