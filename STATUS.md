...
> - **模块 banner 设计总表已出待批**:docs/assets/mockups/模块banner-设计总表.html+png(Frank 问「banner 怎么设计/图哪找/怎么轮/放什么信息/宽高固定?」)——规范:恒 150px 定框 cover 裁剪/Commons 实景 1280×300 三张每模块/B类氛围轮播(crossfade 8s 前景恒定,区别于 news A类内容轮播)/信息=模块名+定位句+2-3 实时数字+CTA/缺图退浅色带零风险。待 Frank 回「按这个做」。
