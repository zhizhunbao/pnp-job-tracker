...
> - **E11-03 Google 登录后端已上线(43212a7,凭据当场办齐)**:GCP 项目 offer2pr/OAuth client 建好(redirect=生产+localhost 两条);后端=302 同意屏+callback(email_verified 必真→邮箱查找/创建→**镜像 Payload3.85 login 签发**:sessions+sid+getFieldsToSign/jwtSign 官方导出;老邮箱账号关联不动密码);本地烟测三项过。**只差 Frank Render 两 env → 自动重部署钮即亮 → 实弹一发**(细节 implementation/E11/03 §4-5):`NEXT_PUBLIC_GOOGLE_CLIENT_ID=343715577783-u3vp2qmcpd1fgcutf2rf2mpsrm0j76ng.apps.googleusercontent.com` + `GOOGLE_CLIENT_SECRET=<GCP 弹窗里的 secret>`。
