...
> - **产品对账文档已立**:[功能价值盘点与数据缺口](docs/功能价值盘点与数据缺口.md)——岗位价值六轴模型 + 数据缺口按痛点 ROI 排序;**下一个已拍板:advisor 公司分析接 Anthropic 服务端 web_fetch**(裸 LLM 不能读 URL;web_fetch 只抓对话中出现的 URL,Haiku 支持基础版,~$0.004/次封顶,注意 prompt 注入面+只在 anthropic 后端启用)。
