...
> - **Google 按钮未亮之谜(交接)**:服务端路由生产 302 全对(env 运行时在);按钮=NEXT_PUBLIC 编译期烤入 client 组件,当前活着的构建疑似还是 env 之前排队的旧 build(今晚 push 密,Render 逐个建;「新版判定」曾被索引提速假信号骗过)。已挂 40min 长轮询盯;**若队列清完仍不亮 → 查 cms/Dockerfile 无 ARG 声明是否挡了 Render 构建期 env 注入**(加 `ARG/ENV NEXT_PUBLIC_GOOGLE_CLIENT_ID` 即修)。亮了才轮到 Frank 实弹。
