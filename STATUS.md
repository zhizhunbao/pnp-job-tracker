...
> - **E6-04 省 PNP 抽选线:代码侧全落地(2026-07-05),只剩生产 DDL(卡用户授权)→ push → seed → 终验**。实现文档 [E6-04](docs/implementation/E6-数据补强/04_省PNP抽选线.md)(§7 有备好的 DDL SQL)。**源盘点实测推翻交接假设**:① 「CRS 对齐可算差分」档全灭——**ON 2026-06-26 OINP 改制**(旧 8 流全删/EOI 关闭/新 Ontario Workforce Priority 流细则待公布)、**AB 2026 换 WEOI 自有分制**(连 EE 流最低分 47-71 也是 WEOI 非 CRS)、SK 2025 改制后无 EOI 抽选;→ 全部走「事实展示」档,match 不加规则,「差 X 分」维持联邦 EE 独有;② BC httpx 200(交接猜的 403 不成立,带 Chrome UA 即通)。链路:`etl/pnp/build_draws.py`(BC 表 rowspan 展开/AB Table10/MB prose/ON notice,挂 pnp 源周更)→ raw/pnp/draws.json(**08 目录驱动天然跳过**,无 occupations 键,实证过)→ mart pnp_draws 25 行(BC8/AB8/MB8/ON通告1)→ 新 PnpDraws collection+seed 白名单 → PnpListSection 顶部 PnpDrawsBlock(**分数带分制标注「非 CRS」**,三语)。`npm run build` 过,已 commit **未 push**(表不存在时 /jobs 的 payload.find 会 500——B4 教训先 DDL 再 push)。**连带发现**:OINP 改制后现有 oinp-in-demand(56 NOC)/oinp-tech(9 NOC)具名 chip 语义存疑(旧流已关,抓取还能解析到旧清单页),已开后台任务卡评估,待新流细则另立项。
