...
> - **Render Free→Starter ✅(2026-07-05,用户绑卡+升级,$7/月)**:实测 /jobs 冷启动 48s→消失(不休眠)、热请求 14-17s→3.6-6.8s(升级前 Free 档 0.1 CPU)。~~剩余性能大头~~ → **E7-04 已落地(见下条)**。
> - **E7-04 /jobs 列表服务端分页 ✅ 代码侧收口(2026-07-05,worktree session,先文档后动手)**:首屏只发默认序前 200 行(document gzip **1.5MB→77.6KB**),筛选/搜索/排序/滚动全走 `/api/jobs` SQL 增量取(查询单点新 `lib/jobsList.ts`;`buildJobsWhere` 加 full 模式全键翻译,alerts 的 lenient 模式不动);弹框 6 维度改 `/api/jobs/dims` idle 懒加载(未到位先 loading 蒙层,不把「没加载」显示成「未列入」);?q=/?prov=/?broad= 回流改 searchParams SSR 直出;免费匹配「默认序前 10 岗」用 id→level map 保住逐位语义(@test.local 实测:前 10 有值/11 起 null/筛选排序视图跟 id 走);「公司信息」弹框在招清单改 `?company=` 精确查。**顺带修 fTeer `Number('TEER 3')=NaN` 老 bug**(保存筛选带 TEER 从来没命中过)。回归:build ✅ + API 语义 10 项 + playwright 滚动/弹框 + 免费墙(wage 剥离/fVs 空)全过。**待部署后回填:生产 TTFB(目标 <1.5s)+ Pro 全量匹配列抽查**(无法自建 Pro 测试号);测试号 e704-match@test.local 留库可清。实现文档 [E7-04](docs/implementation/E7-运维与增长/04_jobs列表服务端分页.md)。
