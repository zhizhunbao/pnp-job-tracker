...
> - **E6-04 省 PNP 抽选线:刚起手,只做了源盘点,代码零改动**。接手要点:① **各省打分体系≠CRS**(BC 200 分制/SINP 110/MB 1000)——档案只有 CRS,「差 X 分」省版只对**发布 CRS 线的 EE 对齐流**可算(OINP Tech Draws 页/AAIP EE 流),其余省只能做「最近抽选事实展示」(弹框/统计/抽选快报素材),诚实分两档;② 已验证 httpx 可通的省源:ontario.ca(有现成 oinp-tech-draws 页抓取!)/alberta.ca/saskatchewan.ca,BC gov 站疑似 403(老坑:需 browser_fetch);③ 模板=`build_ee_draws.py`(IRCC JSON 先例)+ 各 build_<prov>;④ 落库若新建 pnp_draws collection,**勿忘 payload_locked_documents_rels 补列**(B7 教训);更省事的备选=效仿 ee_categories 把最新抽选嵌进现有维度行。按 molit 规矩先写 E6-04 实现文档再动手。
