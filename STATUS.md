...
> **B4 最后一步(用户手动,5 分钟)**:Render Dashboard → pnp-cms → Environment 填 5 个变量(值在本地 `cms/.env` 底部 Stripe 段;webhook secret 用生产 endpoint 专属的 `whsec_u715...`,**不是**本地 stripe listen 打印的那个):`STRIPE_SECRET_KEY` `STRIPE_PRICE_30D` `STRIPE_PRICE_90D` `STRIPE_WEBHOOK_SECRET` `STRIPE_WECHAT_PAY=1` → 部署后生产注册小号跑一笔 4242 复验 = M2 线上确认。
>
