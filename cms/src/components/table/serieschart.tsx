 ...
 * 每行一条线,y = 指数(该行首个有值点 = 100),x = 时间点列的 label;
 * 图例在图上方(色块 + 行名 + 最新原值),每个点带 `<title>` 交代原值。
 ...
 */
import {
  SERIES_ANCHOR_END, SERIES_ANCHOR_MID, SERIES_BASELINE_MID, SERIES_DASH_BOX, SERIES_DASH_H, SERIES_DASH_W,
  SERIES_DOT_R, SERIES_FILL_NONE, SERIES_KEY_SEP, SERIES_LINE_W, SERIES_SVG_ROLE, SERIES_TEXT_X, SERIES_TICK_Y,
} from './constants'
import { seriesPlotOf } from './functions'
 ...
 * @returns 图。
 */
export function SeriesChart<T>({ pointKeys, pointLabels, rows, valueOf, labelOf, words }: SeriesChartIn<T>) {
  const plot = seriesPlotOf({ pointKeys, pointLabels, rows, valueOf, labelOf })
  const legend = []
  const marks = []
  ...
  for (const t of plot.ticks) {
    ticks.push(
      <text key={t.x} className={css.axisText} x={t.x} y={SERIES_TICK_Y} textAnchor={SERIES_ANCHOR_MID}>{t.text}</text>,
    )
  }
  return (
    <div className={css.chart}>
      <div className={css.chartNote}>{words.indexNote}</div>
