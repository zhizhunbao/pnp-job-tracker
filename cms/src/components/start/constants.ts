 ...
 * 2026-09-23 大类重排(27 → 23):每组补上新大类名(高管、人力资源、行政文员、客户服务、财会金融 → 商务办公;
 * 科研 → STEM;公共服务、文体艺术 → 教育文体;酒店旅游、零售销售 → 餐饮零售;机修技工 → 技工;运输物流;农林矿业 → 制造农矿),
 * 旧名等数据层切完再清(过渡期两套都认)。
 */
export const IND_BROADS: Record<string, string[]> = {
   ...
   */
  food: ['餐饮', '住宿', '零售', '销售', '生活服务', '酒店旅游', '零售销售'],

  /**
   * 运输物流:运输、物流。
   */
  transport: ['运输', '物流', '运输物流'],

  /**
   * 制造农矿:制造、农业、矿业三个一二产大类。
   */
  manufacturing: ['制造', '农业', '矿业', '农林矿业'],

  /**
   * 商务办公:管理层、商务、行政、文员、金融、会计、法律七个办公室大类。
   */
  business: ['管理层', '商务', '行政', '文员', '金融', '会计', '法律', '高管', '人力资源', '行政文员', '客户服务', '财会金融'],

  /**
   * 教育文体:教育、社会服务、艺术、体育。
   */
  education: ['教育', '社会服务', '艺术', '体育', '公共服务', '文体艺术'],
}
