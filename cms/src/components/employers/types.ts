...
export type PoolRow = {
  ...
  /**
   * 真总部一行字(街址、市、省码,英文原样;2026-09-20 真总部进库);空串 = 库里没有。
   */
  hq: string
...
export type EmployerCellRow = {
   ...
   * 2026-09-20 起是全部在招地点(岗多的在前),格里默认露前三枚,其余展开才出。
   */
  locs: string[]
   ...
   * 2026-09-20 真总部接上:取池行的 hq(公司表真总部三格拼好的一行:官网页面原句核对过的,官网没标的退 Wikidata);库里没有的照旧空串。
   */
  hqText: string

  /**
   * 总部格的 Google 地图链接(查的就是 hqText 那一行);空串 = 不成链(没有真总部)。
   */
  hqHref: string
  ...
  /**
   * 指定列的字,一行一个项目:「AIP NB、NS」「RCIP Sudbury, ON」(列头已写「指定雇主」,不再重复一枚胶囊 —— 2026-09-13 Frank
   * 「这个胶囊不重复吗」);指定但名单没写项目时退回一行「指定雇主」;空表 = 非指定(渲横杠)。
   * 2026-09-19 Frank「指定雇主格写明所在地」:原先只写「AIP、RCIP」—— 资格是按省 / 按社区给的,旁边又是在招最多的 Toronto,
   * 会让人以为多伦多的岗也能走(Englobe 实拍);09-13「下面就没必要再显示省份了吧」那条由此改判。
   */
  designatedLines: string[]
