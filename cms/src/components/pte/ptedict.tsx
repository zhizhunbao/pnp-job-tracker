 ...
 * 查不到给一句「没查到」)。位置是运行时算出的视口坐标,经 style 进。
 *
 * @author Frank
 * @time 2026-09-03 18:00:00
 */
import { Button } from '@/components/button'
import { DICT_BUSY, DICT_IDLE, DICT_NONE, KIND_LINK, SPACE, TEXT_NONE } from './constants'
  ...
  return (
    ...
    <div className={css.dict} style={dictStyleOf(d.pos)}>
      <div className={css.dictHead}>
        ...
        <Button kind={KIND_LINK} onClick={d.onClose} ariaLabel={t('pte.dict.close')}>{t('pte.dict.close')}</Button>
      </div>
      ...
      <div className={css.dictSrc}>{t('pte.dict.src')}{SPACE}</div>
    </div>
  )
}
