 ...
 * 一义一行(中文界面中文释义,其余英文释义 —— 「如果是韩国人呢」;「中文」标签同日撤);「词形」段。× 关。
 *
 * @author Frank
...
import { bracketOf, dictFormsOf, dictLinesOf, phonOf, spkClsOf } from './functions'
...
export function PteDict({ t, d, lang }: PteDictIn) {
  ...
  if (d.entry != null) {
    let i = 0
    for (const fm of dictFormsOf({ forms: d.entry.forms })) {
      forms.push(
        <div key={i} className={css.dictLine}>
