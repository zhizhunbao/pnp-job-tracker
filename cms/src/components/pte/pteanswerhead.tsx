'use client'
/**
 * 域内小件:答题卡头(英文题型名 + 缩写灰注 + 押题标 | 位置;下一行官方一句指令;
 * RA 准备段再一行倒计时 | 跳过准备)。
 *
 * @author Frank
...
import { KIND_LINK, PHASE_READY } from './constants'
 ...
 * @returns 头两三行。
 */
export function PteAnswerHead({ t, q, type, pos, a }: PteAnswerPartIn) {
    ...
    <>
      <div className={css.qHead}>
        <div>
          ...
          {q.predicted && <span className={css.tag}>{t('pte.hotMark')}</span>}
        </div>
        <div className={css.pos}>{pos}</div>
      </div>
      ...
      {inPrep && (
        <div className={css.prepRow}>
          ...
          <Button kind={KIND_LINK} onClick={a.onSkipPrep}>{t('pte.skipPrep')}</Button>
        </div>
      )}
    </>
  )
