'use client'
/**
 * 域内小件:单题页左侧目录树 —— 上排题型钮(有题的型,亮态 = 清单现在显示的型,点了原地切清单不跳页 —— 「不要跳页面,还是在当前页面筛选」;
...
import { navTextOf, typeLabelOf, typeNameOf } from './functions'
import { usePteNav } from './hooks'
import type { PteNavIn, PteRow } from './types'
 ...
 * @returns 侧栏。
 */
export function PteNav({ types, type, rowsByType, qid, lang }: PteNavIn) {
  const n = usePteNav({ qid, type })
  let rows: PteRow[] = []
  const picked = rowsByType[n.navType]
  if (picked != null) {
    rows = picked
  }
  const chips = []
  for (const x of types) {
    if (x.count > 0) {
      chips.push(
        <Chip key={x.code} onClick={n.pickOf(x.code)} active={x.code === n.navType}>
