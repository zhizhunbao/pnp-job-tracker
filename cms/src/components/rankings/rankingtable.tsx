...
import { isCompanyBoard, noteTextOf, updatedTextOf } from './functions'
 ...
 * 榜单表。
 *
 * @param props 榜 slug、本榜的行与取词函数。
 * @returns 更新时间、口径注与列表区。
 */
export function RankingTable({ slug, items, t }: RankingTableIn) {
  const note = noteTextOf({ t, slug })
  const company = isCompanyBoard(slug)
  return (
    <>
      <div className={css.updated}>{updatedTextOf({ t, items })}</div>
