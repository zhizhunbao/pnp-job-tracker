 ...
 * Frank「没有为什么显示出来」「信息不足不要显示」):「Information insufficient」同族,整句当「没有」,节不出。
 */
export const JD_NONE_RE = /^(\(?\s*(not|none|n\/a)(\s+(stated|specified|mentioned|provided|available|applicable|listed))?\s*\)?|information (is )?not available[^.]*\.?|(information )?insufficient( information)?\.?)$/i
...
// =========================================================================
// 空值(同一个空串在这个域里有十几种意思:没给 / 查过了没有 / 剥掉一段 —— 起名才分得开)
// =========================================================================

/**
 * 入参缺席:URL query、JSON body 没带这一格,或函数入参上游根本没填。
 * 空串在这里的意思是**「调用方没给」**,不是「查过了确实没有」——
 * 后者按被查的东西各有自己的常量(JD_NONE、MAIL_NONE、TITLE_NONE…)。
 * 分开的理由是下游待遇相反:没给 = 这道筛选、这次抓取整个跳过,一分代价都不花;
 * 查过了没有 = 代价已经花过,可以进负缓存不再重试。
 * body 整个解析不了也归这里 —— 拿不到就是没给,与「带了但是空」同样处理。
 * ⚠️ 拿它当初值而不是 null,是为了让后面一句 `x === ''` 判完,
 * 不必再分「键不存在」与「键在但值是空」两种没有。
 * 判空位按闸的钦定写法留字面量,所以全域搜不到一句 `=== PARAM_NONE`,那不是漏改。
 */
export const PARAM_NONE = ''

/**
 * 排序指令缺席:表头没点过(前端不传 sort 与 dir),或点的是 Pro 专属键而当前是免费态。
 * 后者会把键**主动清空**降回默认排序 —— 不报错,也不偷偷按 Pro 的口径排;
 * 于是「没点过」与「点了但不给你排」在 orderByClause 里合成同一条路:回落 ORDER_DEFAULT_COL。
 */
export const SORT_NONE = ''

/**
 * 时间戳的「没有」:ETL 心跳表还没建(全新库第一次跑,pg 报 undefined_table),
 * 或本次结果集一行都没有 —— 两种情况都拿不到「数据更新于」那个时刻。
 * 🔴 不折成当前时间 —— 那等于替 ETL 编一个它没跑过的时刻,页面会显示一条假的新鲜度;
 * 空串下前端只是不显示这一行。
 */
export const STAMP_NONE = ''

/**
 * 库格是 NULL 落到对外形状上的样子(与 db 域 text 词汇表同一口径)。
 * 只对**文本**格这么办:前端拿到就往格子里塞,空串直接渲染成空格,
 * 而 null 会逼每一个消费点再判一次空。
 * ⚠️ 官方可空的**数值**不适用 —— 那种必须保 null(折 0 = 替官方编数)。
 */
export const CELL_NONE = ''

/**
 * 时间格**库里是 NULL** 时,ISO 串那一格对外给的空串(行构造器 iso 的出口)。
 * 🔴 不折成当前时间,也不折成纪元零点 —— 那两样都等于替库编一个它没记过的时刻,
 * 页面会显示一条查无实据的日期。空串下前端只是不显示这一格。
 * 与 STAMP_NONE 分开:那个说的是「ETL 心跳整张表拿不到」,这个说的是「这一行这一格没值」。
 */
export const ISO_NONE = ''

/**
 * 档案里还没找到与本岗同小类、同族的 NOC 码。
 * 匹配引擎按「全码 → 同小类(NOC_MINOR_LEN 位) → 同族(NOC_SUBMAJOR_LEN 位)」逐级降档,得分依次 40 / 30 / 20。
 * 位数写常量名不写数字,是因为那两个常量才是真相(小类 4 位、族 3 位),抄一遍就多一份会跑偏的副本。
 * 空串是「这一档还没命中」的中间态:两档都扫完仍是空串,才落到不匹配(0 分,并置 nocMiss)。
 */
export const NOC_NONE = ''

/**
 * 省名认不出来:筛选参数收进来的可能是全名(`British Columbia`),也可能已经是省码;
 * 全名表里查不到就给空串,让调用方按「这不是省名」继续把它当搜索词处理。
 * 🔴 不猜、不做模糊匹配 —— 猜错会把整页结果筛到另一个省去。
 */
export const PROV_CODE_NONE = ''

/**
 * 这个错不是 Postgres 抛的(对象上没有 code 那一格),于是没有错误码可比。
 * 空串保证调用方 `pgCodeOf(e) === PG_UNDEFINED_TABLE` 这类比较**恒假** ——
 * 认不出的错原路抛出去,不会被当成「表不存在」悄悄咽掉。
 */
export const PG_CODE_NONE = ''

/**
 * 这个来源没有被实测拦截过(不在 BLOCKED_SRC 表里)。
 * 空串 = 前端不出「该站拒绝本站自动读取」那句陈述;有站名才出,
 * 所以这一格绝不能兜底成某个站名 —— 冤枉一个没拦过的站等于替它道歉。
 */
export const BLOCKED_SRC_NONE = ''

/**
 * 可选 SQL 片段「不拼」:空串拼进模板等于什么都没加,不是拼进一个空列。
 * 用处是同一条 SQL 按场景多带、少带一列 —— 如职业搜索只在结果条数够少时才算工资中位,
 * 条数多了就把那段 SELECT 留空,省掉一次全表聚合。
 */
export const SQL_SEG_NONE = ''

/**
 * 职业名维度表里没有这个 NOC 码(码是从别的表算出来的,名字表未必跟得上)。
 * 三语各自留空,前端照旧显示码 —— 宁可只给码,也不拿英文名冒充中文名、韩文名。
 */
export const OCC_TITLE_NONE = ''

/**
 * 裁 JD 头部时「前面还没留下过行」(已留下的头部还是空的)。
 * 它只喂给「上一行是不是冒号结尾」那个判断:空串测不出冒号,
 * 于是开头的短句拿不到「上一行是标题」这层豁免,照孤行规则丢掉。
 */
export const PREV_LINE_NONE = ''

/**
 * 抓不到 HTML:URL 过不了 badHost 那道 SSRF 闸(协议不是 http/https,或主机落在内网与回环网段),
 * 响应非 2xx,超时,连接被拒。
 * 这几种失败在调用方眼里是同一件事(没有页面可解析),所以合成一个空串;
 * 具体是哪一种已经进了日志,不靠返回值区分。
 */
export const HTML_NONE = ''

/**
 * 没有 JD 正文:没给投递链接、页面抓不到、抽出来的正文短于 JD_MIN_LEN(多半是壳页或验证页)。
 * 空串是**免费**的失败态 —— 前端空态照旧引导去官方原帖,不报错、不挡住整个弹框;
 * 它同时是负缓存里记下的那个结果,一段时间内不再重复打原站。
 */
export const JD_NONE = ''

/**
 * Job Bank 这条职位没有指向原站的外链:本站抓到的这页就是终点,
 * 正文只能取 Job Bank 自己那份,不再跳第二跳。
 */
export const JB_LINK_NONE = ''

/**
 * 没有岗名:标题标签取不出,或剖开后每一段都被判成垃圾(域名、站名、招聘平台后缀)。
 * 它同时是「挑最长的一段」那个循环的起点 —— 长度 0,第一个候选必然胜出,
 * 于是不必为「还没有候选」另写一条分支。
 */
 ...
 * 或这条通道名不在三语表里(官方新开的通道,表还没跟上)。
 * 空串 = 这一格什么都不加,英文原名照旧显示。
 */
export const STREAM_NOTE_NONE = ''

/**
 * 剥字用的替换串:`replace(某某_RE, STRIP_REPL)` 里空串的意思是把匹配到的整段**删掉**,
 * 不是替成一个空格。它和上面那些「没有值」的空串不是一回事 —— 那些是结果,这个是操作:
