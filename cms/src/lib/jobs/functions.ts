...
export function toJdFormattedCell(r: Row): MaybeStr {
  const cell = textOrNull(r.jd_formatted)
  if (cell == null) {
    return null
  }
...
/**
 * 整理版的节标记一律顶到行首(2026-09-14 Frank「跑偏了」):模型偶发把五节挤成一行,翻译按行对位就整篇塞进首节;
 * 写库前与读库后都过一遍,存量 310 条单行整理版读出来也是五行。
 *
 * @param text 整理版文本。
 * @returns 每个节标记都在行首的整理版。
 */
...
/**
 * 同 jdMarkLinesOf,格子可空。
 *
 * @param cell 整理版;null = 还没整理。
 * @returns 顶好行首的整理版;null 照旧。
 */
function jdMarkLinesOrNull(cell: MaybeStr): MaybeStr {
  if (cell == null) {
    return null
  }
  return jdMarkLinesOf(cell)
