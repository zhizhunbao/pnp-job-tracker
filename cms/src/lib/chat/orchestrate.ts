...
export async function orchestrate(
  rawPool: any, input: {
    ...
    /** 抽不出职业码时的第二次机会(lib/agent 注入;不传 = 照旧反问)。
  ...
  const slots: Slots = {
    ...
    // agent 兜出来的省份只在用户原话里一个省都没抽到时才用（手抽优先，同 D3「只补空」）
    ...(rescuedProvs.length && !draft.provs.length ? { provs: rescuedProvs } : {}),
  }
