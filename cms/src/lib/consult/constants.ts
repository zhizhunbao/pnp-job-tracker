...
/**
 * 一条事实**没有可指的官方链接**时,`evidence.url` 填的空串。
 *
 * 四态里的「本站未收录 / 不适用 / 无此项」都走它:我们手上根本没有那一页,
 * 编一个 URL 出来比不给更糟(用户点开发现驴唇不对马嘴,整封答复的可信度一起塌)。
 *
 * 拿到它的只有**本域自己造的**那种空壳事实;从库里取回来的行拿不到 ——
 * `lookupDraws` / `lookupEe` / `lookupPermit` / `lookupPoints` 都先按 `evidence.url`
 * 筛过一道,无出处的整行不返回(铁律:URL → 数据 → SQL)。所以这个空串永远配
 * `value: null`,「有个数字却没有出处」这种组合根本拼不出来 ——
 * 出口闸的 `allowedNumbers` 只看 value / valueText / label,不读 evidence,
 * 靠的就是上游这一刀。
 */
export const EVIDENCE_URL_NONE = ''

/**
 * 一条事实**没有抓取时刻**时,`evidence.fetched` 填的空串。
 *
 * 与 `EVIDENCE_URL_NONE` 分开是因为两者会各自单独出现:在招岗位那条事实
 * (`jobsFacts`)链接指得回本站职位板、`url` 是实的,但那个数是**当场从库里数出来的**,
 * 没有「某天抓的官方页」这回事,`fetched` 就只能空。
 * 反过来「本站未收录」那类两格同时空。
 */
export const EVIDENCE_FETCHED_NONE = ''

/**
 * 计分表查询(`SQL.EE_POINTS_GRID`)里「这一维不筛」的参数值。
 *
 * 那条 SQL 每个可选筛选位都写成 `$n = '' OR col = $n`,传空串就是这一维整个不参与筛选 ——
 * 所以这里的空串不是「没取到值」,是**一个明确的查询决策**:节号、行类型、factor、criterion
 * 四位里没点名的那几位一律放开。
 */
export const SQL_NO_FILTER = ''

/**
 * 一段**可选文案不出**时的空串:`seg()` 条件不成立的返回,以及 tier / blocked / missing /
 * 前情摘要 / 重写提示这些「有才拼、没有就整段消失」的初值。
 *
 * 全站禁三目之后,「x 有值才拼这段」只剩「先给空串、成立再覆盖」这一种写法;
 * 空串拼进句子等于零宽度,读的人看不出这里曾经可能有一段。
 */
export const SEG_NONE = ''

/**
 * 四态事实(`statusFact`)的 `valueText`:这类事实**根本没有值**,不是值为零。
 *
 * 「本站未收录」「不适用」都属于此 —— 出口闸的数字校验靠 `value === null` 放它过,
 * 这一格跟着留空,免得渲染层把一个空壳数字摆进句子里。
 */
export const VALUE_TEXT_NONE = ''

/**
 * 模型**没给这个工具参数**时的空串(职业码、计分表节号)。
 *
 * 工具入参由模型现填,少填一格是常态而不是错:少了职业码就不改这一趟的采信,
 * 少了节号就取整张表的 summary 行。空串在这儿的意思是「它没说」,
 * 不是「它说了空的」 —— 所以一律 `=== ''` 显式判,不看真假值。
 */
export const ARG_NONE = ''

/**
 * `replace()` 的替换串:换成空串 = 把匹配到的那段**删掉**(千分位逗号、末尾的 `.0`)。
 *
 * 出口闸比对数字前要先把两边规范成同一形态,`1,200` 与 `1200`、`5.0` 与 `5`
 * 必须算同一个数,否则模型照抄事实里的数也会被判成「编的」。
 */
export const STRIP_SUB = ''

/**
 * 同一条消息里多个文本块 `join` 的分隔符:一个字都不插。
 *
 * 模型把一段话切成几个 text 块纯属传输分片,块与块之间**本来就没有边界** ——
 * 插空格或换行会在句子中间凭空多出一个空隙,中文句尤其明显。
 */
export const TEXT_BLOCK_SEP = ''

/**
 * `textOf` 拿到的这条消息**不是助手正文**(思维链、工具调用)时的返回。
 *
 * 空串在这里等于「这条没有正文可取」,调用方 `lastDraftOf` 靠它跳过这条接着看下一条。
 * 与「助手答了但答了个空」区分不开是可以接受的:两者都进不了 `drafts`,
 * 一条都没进就由 `lastDraftOf` 抛 `FAIL_MSG.emptyDraft`,轮不到出口闸去判。
 */
export const TEXT_NONE = ''

/**
 * 收件箱刚建起来、**还没取到职业名**时 `title` 的初值。
 *
 * 档案里没预填职业码时整个查询都不做,名字要等模型调 `search_occupations`
 * 采信出一个码之后由 `nocOf` 回填 —— 这一格空着是**待补**,不是「这个码没有名字」。
 */
export const TITLE_PENDING = ''

/**
 * 一趟对话**还没起草出答复**时 `answer` 的初值。
 *
 * 它一定会被覆盖:`consult` 的重写循环 `attempt <= GUARD_RETRIES` 至少跑一圈,
 * 每圈开头就把 `draftOnce` 的结果赋进来(出口闸撞了才重写,所以可能覆盖好几次);
 * 起草不出来的那条路是 `lastDraftOf` 直接抛错,走不到读 `answer` 的地方。
 *
 * 所以这一格纯粹是**声明处的占位**:宪法禁 `undefined` 与 `?`,`let answer: string`
 * 不给初值没法声明,于是给空串,而不是给一句可能被当成真答复漏出去的文案。
 */
export const ANSWER_PENDING = ''
...
/**
 * 请求体里**没带正文**时 `text` 的初值。
 *
 * body 解析失败、字段缺席、类型不对三种情况都收成同一个空串 ——
 * 对这条路由来说它们是同一件事:没有可答的问题,后面统一拒掉,
 * 不必让三种坏输入各走一条分支。
 */
export const INPUT_TEXT_NONE = ''

/**
 * 回给前端的 `slots.occText`:这条路由**不回填职业名文本**。
 *
 * `ChatSlots.occText` 的契约是 `string`(不许 null),而 `consult` 只回
 * `{ answer, facts, noc, degraded }` —— 职业名留在域内的收件箱(`Inbox.title`)里,
 * 没有出到路由这一层。所以这一格恒为空串,与 `ChatSlots.occText` 上
 * 「新链暂不产,恒空串」说的是同一件事;要它有值得先让 `consult` 把 `title` 一起回出来,
 * 不是在这儿临时补查一次。
 */
export const OCC_TEXT_NONE = ''
