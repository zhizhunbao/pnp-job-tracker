 ...
 * （纯移动，只换门禁/分片选路/拿池三处为域函数）；写法存量进 suppressions
 * 基线，形制化另批金标对拍 —— 它是全站最危险的一条路（直灌生产库）。
 * 原 seed 文件头的设计史：
 *
...
import {
  BYTE_LBRACKET, BYTE_RBRACKET, BYTE_WS_MAX, E_BAD_GZIP, E_BAD_NAME, E_NOT_ARRAY, GZIP_MAGIC_0, GZIP_MAGIC_1,
  P_TOKEN, T_UNAUTHORIZED, TABLE_NAME_RE, TMP_PREFIX, TMP_SUFFIX,
} from './constants'
import { martCounterpart, martHash, martPaths, martTmpDir, seedTokenOk } from './functions'
import type { MartRow, MartValue } from './types'
...
export async function martUploadRoute(req: Request, ctx: { params: Promise<{ name: string }> }): Promise<Response> {
  ...
  if (TABLE_NAME_RE.test(name) === false) {
    return Response.json({ ok: false, error: E_BAD_NAME }, { status: 400 })
  }
  ...
  try {
    ...
    return Response.json({ ok: false, error: E_BAD_GZIP + msg }, { status: 400 })
  }
  let head = 0
  let tail = body.length - 1
  while (head < body.length && body[head]! <= BYTE_WS_MAX) {
    head++
  }
  while (tail > head && body[tail]! <= BYTE_WS_MAX) {
    tail--
  }
  if (body.length === 0 || body[head] !== BYTE_LBRACKET || body[tail] !== BYTE_RBRACKET) {
    return Response.json({ ok: false, error: E_NOT_ARRAY }, { status: 400 })
  }
  const dir = martTmpDir()
  fs.mkdirSync(dir, { recursive: true })
  const tmp = dir + '/' + TMP_PREFIX + name + '.json' + TMP_SUFFIX
  fs.writeFileSync(tmp, body)
  fs.renameSync(tmp, dir + '/' + name + '.json')
...
type Row = MartRow
type PgClient = DbClient   // 形状归 lib/db 的 types 管;事务体照旧不动(BEGIN/COMMIT/ROLLBACK/finally release)
...
export async function seedRoute(req: Request): Promise<Response> {
  ...
  // mart 读取链与 `__meta` 分片形制：单一来源在 lib/mart/functions（martPaths/martHash，
  // 2026-08-23 收拢；事故防线语义——目录全无/分片半程招错回滚、单表缺文件回空表——原样在彼）。
  ...
  const client: PgClient = await (await getDb()).connect()
  let closed = 0
  ...
  try {
    ...
    if (martPaths('stats_daily').length > 0) {
      const dailyRows = mart('stats_daily').map((r: any) => ({
