...
function toEmployerPool(r: MartRow): MartRow {
  return {
    ...
    province: cellOf(r.province), city: cellOf(r.city), locations: jsonTextOf(r.locations),
    loc_provs: jsonTextOf(r.locProvs), designated: truthyOf(r.designated),
...
/**
 * macro_series 行(宏观时间序列的一个点)。⚠️ value 走 cellOf 保 null ——
 * 官方缺位的点在 ETL 侧根本不出行,这里更不许折 0(折 0 = 替官方编数)。
 *
 * @param r mart 行。
 * @returns 库行。
 */
function toMacroSeries(r: MartRow): MartRow {
  return {
    geo: cellOf(r.geo), key: cellOf(r.key), period: cellOf(r.period), freq: textOf(r.freq),
    value: cellOf(r.value), as_of: textOf(r.asOf), unit: textOf(r.unit), source: textOf(r.source),
    fetched: textOf(r.fetched),
  }
}
...
/**
 * dli 行(E12-03;2026-09-12 粒度改校 × 城一行 + name_zh 人工核定译名列 + QS 两格,
 * Frank「dli 院校没有大学吗」「大学名 最好也加上中文翻译吧」「再加上 qs 排名」;
 * kind 院校种类列 2026-09-12 Frank「这个应该加一个 大学 和 学院的 筛选吧」)。
 *
 * @param r mart 行。
 * @returns 库行。
 */
function toDli(r: MartRow): MartRow {
  return {
    ...
    qs_rank: cellOf(r.qsRank), qs_rank_display: cellOf(r.qsRankDisplay), kind: cellOf(r.kind),
...
/**
 * pte_sentences 行(2026-09-04 整句中文)。
 *
 * @param r mart 行。
 * @returns 库行。
 */
function toPteSentence(r: MartRow): MartRow {
  return { qid: cellOf(r.qid), idx: cellOf(r.idx), en: cellOf(r.en), zh: cellOf(r.zh) }
}
...
function toStat(r: MartRow): MartRow {
  ...
  return {
    ...
    wage_low_hourly: cellOf(r.wageLowHourly), wage_med_hourly: cellOf(r.wageMedHourly),
    wage_high_hourly: cellOf(r.wageHighHourly),
...
export function toCompany(x: ToCompanyIn): MartRow {
  ...
  return {
    ...
    hq_address: cellOf(x.r.hqAddress), hq_city: cellOf(x.r.hqCity), hq_province: cellOf(x.r.hqProvince),
    hq_quote: cellOf(x.r.hqQuote), hq_source: cellOf(x.r.hqSource), hq_parent: cellOf(x.r.hqParent),
    site_checked_at: cellOf(x.r.siteCheckedAt),
...
export function dimSpecs(): DimSpecs {
  return [
    ...
    { table: TBL_MACRO_SERIES, cols: COLS_MACRO_SERIES, toRow: toMacroSeries },
    ...
    { table: TBL_PTE_SENTENCES, cols: COLS_PTE_SENTENCES, toRow: toPteSentence },
