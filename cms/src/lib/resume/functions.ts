...
import {
  BRACE_CLOSE, BRACE_OPEN, CLAMP, ERR_SLICE, ERR_UNSUPPORTED, EXT_DOCX, EXT_PDF, EXT_SEP, FREE_ROWS,
  NOTE_MAX, REQ_MAX, ROLE_SYSTEM, ROLE_USER, ROWS_MAX, ROWS_MIN,
} from './constants'
import { MATCH_REWRITE, MATCH_SYSTEM, MATCH_USER, OUT_LANG, OUT_LANG_DEFAULT } from './prompts'
import type {
  CaughtError, ExtractIn, ExtractOut, Gated, GateMatchIn, JsonObj, MatchMessages, MatchPromptIn,
  MatchRows, MaybeMatchRows, ParsedJson,
} from './types'
...
export function matchPrompt(input: MatchPromptIn): MatchMessages {
  const hit = OUT_LANG[input.lang]
  let outLang = OUT_LANG_DEFAULT
  if (hit != null) {
    outLang = hit
  }
  let rewrite = ''
