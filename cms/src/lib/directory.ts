...
export type LmiaRow = { name: string; region: string; website: string; lmiaPositions: number; lmiaStreams: string; lmiaLastQuarter: string }
...
export async function fetchLmiaEmployers(pool: any, { q, page }: { q: string; page: number }) {
  ...
  const { rows } = await pool.query(
    `SELECT name, region, website, lmia_positions, lmia_streams, lmia_last_quarter FROM companies ${where}
     ORDER BY lmia_positions DESC, name ASC LIMIT ${DIR_PAGE_SIZE} OFFSET ${Math.max(0, page) * DIR_PAGE_SIZE}`, args)
  const items: LmiaRow[] = rows.map((r: any) => ({
    name: r.name ?? '', region: r.region ?? '', website: r.website ?? '',
    lmiaPositions: Number(r.lmia_positions) || 0, lmiaStreams: r.lmia_streams ?? '', lmiaLastQuarter: r.lmia_last_quarter ?? '',
  }))
  return { items, total: n as number }
}
