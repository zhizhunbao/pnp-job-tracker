...
const JOB_COLUMNS = `j.id, j.title, c.name AS company_name, c.slug AS company_slug, c.address AS company_address, c.description AS company_description, c.sectors AS company_sectors,
  ...
  j.noc, j.category, j.teer, j.broad, j.mid, j.fine, j.accessibility, j.score, j.grade_channel, j.pnp_eligible, j.pnp_stream, j.ee_category, j.aip, j.pilot, j.pilot_community, j.pilot_employer,
...
export function mapJobRow(j: any, pro: boolean, matchLevel: JobRow['match']): JobRow {
  return {
    ...
    aip: !!j.aip,
    pilot: j.pilot ?? '', pilotCommunity: j.pilot_community ?? '', pilotEmployer: !!j.pilot_employer,
