 ...
 */
export const EMPLOYER_POOL_BY_SLUG = `SELECT designated, designated_programs, designated_provinces, designated_places FROM employer_pool WHERE slug = $1 LIMIT 1`
 ...
 */
export const EMPLOYER_POOL_BY_KEY = `SELECT name, industry, province, designated, designated_programs, designated_provinces, designated_places, lmia_last_quarter FROM employer_pool WHERE key = $1 AND slug IS NULL LIMIT 1`
 ...
 * 2026-09-13 晚 /fe 雇主页:主场城市 LEFT JOIN cities 带回人工核定译名 city_zh / city_ko(地点列接 09-11 城市显示拍板,
 ...
 *   「清了」,后两个参数各往前挪一位。)
 * $11=在招 EE 类别或 ''(2026-09-19「全部类别」筛选;GIN 索引 employer_pool_ees_idx)。
 * $12=公司分类键或 ''(2026-09-19 晚 Frank「应该单独弄一个公司的分类。和雇主类型联动」:私营 = 模型判的
...
export const employerPoolPage = (order: string) => `
    SELECT p.key, p.slug, p.name, p.industry, p.sector, p.province, p.city, p.district, p.ees, p.locations, p.designated,
