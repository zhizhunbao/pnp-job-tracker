...
import {
  COST_CACHE_READ, COST_CACHE_WRITE, COST_INPUT, COST_OUTPUT, FN, LIKE_SPECIAL, LOG, MAX_INPUT_CHARS,
...
import type {
  ...
  InCandidatesOut, Inbox, IsKnownProvIn, IsKnownProvOut, LogFailureIn, LogFailureOut, MakeToolsIn, MakeToolsOut,
...
// 出错就留痕:兜底不许静默降级,更不许把主路径带崩(铁律①的落地口)。
function logFailure(input: LogFailureIn): LogFailureOut {
  console.log(`${LOG.failedHead}${input.fn}${LOG.failedTail}${input.reason}`)
}
...
async function searchCandidates(input: SearchCandidatesIn): SearchCandidatesOut {
  try {
    ...
    logFailure({ fn: FN.searchCandidates, reason: e instanceof Error ? e.message : String(e) })
    return []
  }
}
...
// 🔴 铁律①:兜底失败 = 当它没发生过 —— 任何错都吞掉回 false,不许把主路径带崩。
async function runLoop(input: RunLoopIn): RunLoopOut {
  try {
    ...
    logFailure({ fn: FN.runLoop, reason: e instanceof Error ? e.message : String(e) })
    return false
  }
}

// 🔴 三条铁律:① 永不抛异常(兜底搞崩主路径更糟) ② 只补槽位不产事实 ③ 默认关着(AGENT_FALLBACK=1)。
function agentFallbackOn(): AgentFallbackOnOut {
  return process.env.AGENT_FALLBACK === '1'
}

// 用户原话 → 槽位(或 null=agent 也没辙);返回 null 的每条路都必须让调用方回到原来的反问。
export async function resolveByAgent(input: ResolveByAgentIn): ResolveByAgentOut {
  try {
    ...
    if (out.gaveUp || !out.claim) {
      console.log(`${LOG.noSlots}${out.gaveUp}${LOG.ms}${ms}${LOG.reask}`)
      return null
    }

    const noc = acceptNoc({ raw: out.claim.noc, candidates: out.candidates })
    if (out.claim.noc !== null && !noc) {   // 编了一个候选里没有的码 —— 不采信,回落反问
      console.log(`${LOG.rejected}${out.claim.noc}${LOG.rejectedWhy}${LOG.ms}${ms}${LOG.reask}`)
      return null
    }
    ...
    console.log(`${LOG.resolved}${shownNoc}${LOG.provs}${shownProvs}${LOG.ms}${ms}${LOG.reason}${reason}`)
    return { noc, provs, reason, ms }
  } catch (e) {
    logFailure({ fn: FN.resolveByAgent, reason: e instanceof Error ? e.message : String(e) })
    return null
  }
}
