...
import { getDesignatedEmployers, getVerdictData } from './verdictCache'
import { matchDesignation } from './designationMatch'
import {
  tripleVerdict,
  type TripleCard, type TripleCompany, type TripleJob, type TripleProfile,
} from './tripleVerdict'
import type { EmployerFacts } from './employerVerdict'
...
export async function buildTripleWire(id: number, answers: ClientAnswers): Promise<TripleWire | { error: string; status: number }> {
  ...
  const profile: TripleProfile = {
    ...
    frenchOk: typeof a.frenchOk === 'boolean' ? a.frenchOk : null,
