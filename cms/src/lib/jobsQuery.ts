...
export function buildJobsWhere(filters: Record<string, unknown>, startIndex = 1): JobsWhere {
  ...
  // 职位类型(E6-06):gig=兼职∪临时/季节(组合口径面向低门槛人群,与前端谓词同一把尺)
  if (s('fEmp') === 'full') conds.push(`j.employment_hours = 'full'`)
  if (s('fEmp') === 'part') conds.push(`j.employment_hours = 'part'`)
  if (s('fEmp') === 'gig') conds.push(`(j.employment_hours = 'part' OR j.employment_term IN ('casual','seasonal'))`)
