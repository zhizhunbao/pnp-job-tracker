 ...
 * district 传了 → 附带本区在招数。市级 2,346 城不预计算(懒化透镜:不进筛选/排序的数据不进 ETL)。
 */
...
export async function GET(req: Request) {
  ...
  const [agg, broads, dli, aip, dist] = await Promise.all([
    ...
    district
      ? pool.query(`SELECT COUNT(*)::int AS n FROM jobs j WHERE j.district = $3 AND j.city = $1 AND j.province = $2 AND ${open}`, [city, prov, district])
      : Promise.resolve(null),
  ])
  const dliCount = await pool.query(`SELECT COUNT(*)::int AS n FROM dli WHERE city = $1 AND province = $2`, [city, prov])
  const a = agg.rows[0] || {}
  return Response.json({
    ...
    districtOpen: dist ? (dist.rows[0]?.n ?? 0) : null,
  })
}
