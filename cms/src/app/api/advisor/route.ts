...
LANGUAGE: the phase label of every step must be written in the OUTPUT language, never in English (unless the output language IS English). Phases mean things like "from now through the first month", "once the offer is in hand", "after nomination" — these are DESCRIPTIONS of what a phase is, NOT strings to copy; translate the idea, never reproduce this English wording verbatim in a non-English answer.`
