...
export async function GET(req: Request) {
  ...
  if (profileOk) matchDims = await loadMatchDims(pool)
  ...
  if (sp.get('view') === 'match') {
    ...
    const m = await fetchMatchPage({ db: pool, pro, profile, matchDims, page, pageSize: PAGE_SIZE, sort: { key: sort.key ?? '', dir: sort.dir ?? '' } })
  ...
  const { jobs, total, updatedAt } = await fetchJobsPage({
    db: pool, pro, profile, profileOk, matchDims, filters: filters as import('@/lib/jobs/types').JobsFilters,
    sort: { key: sort.key ?? '', dir: sort.dir ?? '' }, page, pageSize: PAGE_SIZE,
  })
