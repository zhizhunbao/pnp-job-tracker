          ...
          <div style={{ fontSize: 13, color: '#374151', display: 'flex', flexWrap: 'wrap', gap: '2px 18px', marginBottom: 8 }}>
            {job.company ? (
              <span><span style={metaK}>{t('col.company')}</span>
                ...
                {job.sponsorGrade != null && <span style={{ marginLeft: 6, fontSize: 10.5, padding: '1px 7px', borderRadius: 999, background: '#eff6ff', border: '1px solid #bfdbfe', color: '#1d4ed8', whiteSpace: 'nowrap' }}>{t('gr.sponsorPill', { g: job.sponsorGrade })}</span>}
              </span>
            ) : null}
