...
const zh: Dict = {
  ...
  // E12-09 BC 注册打分自评(分值全部来自官方分值表,前端不编分)
  'sirs.title': 'BC 注册打分(自算)', 'sirs.official': '官方分值表',
  'sirs.f.work': '工作经验', 'sirs.f.education': '学历', 'sirs.f.language': '语言 CLB', 'sirs.f.wage': '时薪', 'sirs.f.area': '工作地区',
  'sirs.fromJob': '按本岗自动算', 'sirs.total': '合计',
  'sirs.above': '比最近一次抽选线高 {gap} 分(最近 {date} {stream},最低 {score} 分)',
  'sirs.below': '比最近一次抽选线低 {gap} 分(最近 {date} {stream},最低 {score} 分)',
  'sirs.note': '按 BC 官方分值表自算(指南 {d} 生效),不是资格认定;官方以文件审核为准。',
...
const en: Dict = {
  ...
  'sirs.title': 'BC registration score (self-calculated)', 'sirs.official': 'Official points table',
  'sirs.f.work': 'Work experience', 'sirs.f.education': 'Education', 'sirs.f.language': 'Language CLB', 'sirs.f.wage': 'Hourly wage', 'sirs.f.area': 'Work area',
  'sirs.fromJob': 'from this job', 'sirs.total': 'Total',
  'sirs.above': '{gap} above the latest cut-off ({date} {stream}, min {score})',
  'sirs.below': '{gap} below the latest cut-off ({date} {stream}, min {score})',
  'sirs.note': 'Self-calculated with B.C.’s official points table (guide effective {d}). Not an eligibility ruling — the BC PNP decides on documents.',
...
const ko: Dict = {
  ...
  'sirs.title': 'BC 등록 점수(자가 계산)', 'sirs.official': '공식 점수표',
  'sirs.f.work': '경력', 'sirs.f.education': '학력', 'sirs.f.language': '언어 CLB', 'sirs.f.wage': '시급', 'sirs.f.area': '근무 지역',
  'sirs.fromJob': '이 공고 기준', 'sirs.total': '합계',
  'sirs.above': '최근 커트라인보다 {gap}점 높음({date} {stream}, 최저 {score}점)',
  'sirs.below': '최근 커트라인보다 {gap}점 낮음({date} {stream}, 최저 {score}점)',
  'sirs.note': 'BC 공식 점수표로 자가 계산({d} 시행 가이드). 자격 판정이 아니며 서류 심사는 BC PNP가 합니다.',
