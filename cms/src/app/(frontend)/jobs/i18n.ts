...
const zh: Dict = {
  ...
  'ps.title': '省提名自评打分', 'ps.official': '官方分值表', 'ps.you': '你的条件(填一次,各省按各自官方表折算)',
  ...
  'ps.range': '该省按通道分别设线,近 {n} 次 {lo}-{hi} 分;本岗通道近期无抽选,不给差分结论',
...
const en: Dict = {
  ...
  'ps.title': 'PNP self-assessment score', 'ps.official': 'official points table', 'ps.you': 'Your profile (fill once, scored by each province’s own table)',
  ...
  'ps.range': 'Cut-offs are set per stream here: {lo}-{hi} across the last {n} draws. No recent draw for this job’s stream, so no gap is shown.',
...
const ko: Dict = {
  ...
  'ps.title': '주정부 이민 자가 점수', 'ps.official': '공식 점수표', 'ps.you': '내 조건(한 번만 입력, 주별 공식표로 각각 환산)',
  ...
  'ps.range': '이 주는 스트림별로 커트라인이 다릅니다(최근 {n}회 {lo}~{hi}점). 이 공고의 스트림은 최근 추첨이 없어 점수 차이는 표시하지 않습니다.',
