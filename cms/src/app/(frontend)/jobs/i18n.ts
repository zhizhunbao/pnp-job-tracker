...
const zh: Dict = {
  ...
  'chart.back': '← 返回', 'chart.drillHint': '点条形可下钻', 'nav.account': '账户',
  // JD 键值段表格化 + 规则解读(2026-07-11 用户拍板:表格+解读列;解读=本地规则口径,零 LLM 不编造)
  'jdi.field': '字段', 'jdi.value': '值', 'jdi.read': '解读',
  'jdi.fte': '全时当量 {v} ≈ 每周约 {h} 小时(按 37.5h/周);移民经验通常按小时折算(EE 全职口径 30h/周)', 'jdi.fteFull': '1.0 = 全职时数',
  'jdi.partTime': '兼职:移民通道的工作经验按小时折算(EE 全职口径 = 30h/周),累积比全职慢', 'jdi.fullTime': '全职:符合多数移民通道的全职经验口径(≥30h/周)',
  'jdi.union': '工会岗:薪级按集体协议公开透明,外籍员工同工同酬', 'jdi.weeksAway': '距今约 {w} 周——留给执照/工签流程的窗口',
  'jdi.payHourly': '按 37.5h/周折年薪约 ${lo}K–${hi}K', 'jdi.payVsMed': ';该 NOC×省 ESDC 中位年薪约 ${m}K',
  'jdi.licence': '硬门槛:执照/认证类要求;国际资质通常需先完成认证对齐', 'jdi.education': '学历门槛;海外学历一般需 ECA/专业机构认证对齐',
  // 大分类(数据值→显示)
...
const en: Dict = {
  ...
  'chart.back': '← Back', 'chart.drillHint': 'Click a bar to drill down', 'nav.account': 'Account',
  'jdi.field': 'Field', 'jdi.value': 'Value', 'jdi.read': 'What it means',
  'jdi.fte': 'FTE {v} ≈ about {h} h/week (at 37.5 h/wk); immigration experience usually counts by hours (EE full-time = 30 h/wk)', 'jdi.fteFull': '1.0 = full-time hours',
  'jdi.partTime': 'Part-time: immigration experience counts by hours (EE full-time = 30 h/wk), so it accrues slower', 'jdi.fullTime': 'Full-time: meets the full-time experience bar most streams use (≥30 h/wk)',
  'jdi.union': 'Union job: the pay grid is public under the collective agreement; foreign workers get equal pay', 'jdi.weeksAway': 'About {w} weeks out — the window left for licensing/permit steps',
  'jdi.payHourly': 'About ${lo}K–${hi}K/yr at 37.5 h/wk', 'jdi.payVsMed': '; ESDC median for this NOC × province ≈ ${m}K/yr',
  'jdi.licence': 'Hard gate: licence/certification required; international credentials usually need recognition first', 'jdi.education': 'Education bar; foreign degrees generally need ECA / professional-body recognition',
...
const ko: Dict = {
  ...
  'chart.back': '← 뒤로', 'chart.drillHint': '막대를 클릭하면 드릴다운', 'nav.account': '계정',
  'jdi.field': '필드', 'jdi.value': '값', 'jdi.read': '해석',
  'jdi.fte': 'FTE {v} ≈ 주 약 {h}시간(주 37.5시간 기준); 이민 경력은 보통 시간 단위로 환산(EE 풀타임 = 주 30시간)', 'jdi.fteFull': '1.0 = 풀타임',
  'jdi.partTime': '파트타임: 이민 경력은 시간으로 환산(EE 풀타임 = 주 30시간)되어 풀타임보다 느리게 쌓입니다', 'jdi.fullTime': '풀타임: 대부분 이민 통로의 풀타임 경력 기준(주 30시간 이상) 충족',
  'jdi.union': '노조 사업장: 급여표가 단체협약으로 공개되며 외국인도 동일 임금', 'jdi.weeksAway': '약 {w}주 후 — 면허/허가 절차에 남은 기간',
  'jdi.payHourly': '주 37.5시간 기준 연봉 약 ${lo}K–${hi}K', 'jdi.payVsMed': '; 해당 NOC×주 ESDC 중위 연봉 약 ${m}K',
  'jdi.licence': '필수 관문: 면허/자격 요건; 해외 자격은 보통 인증 절차가 먼저 필요합니다', 'jdi.education': '학력 요건; 해외 학위는 일반적으로 ECA/전문기관 인증이 필요합니다',
