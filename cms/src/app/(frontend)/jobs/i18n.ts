...
const zh: Dict = {
  ...
  'fact.medianSrc': 'ESDC 开放数据 · 同 NOC × 本省', 'fact.noMedian': '该 NOC × 本省暂无 ESDC 中位数据', 'fact.medianPro': '当地中位工资对比为 Pro 功能', 'fact.vsNote': 'vs 中位 = 本岗年薪 ÷ 当地中位 − 1',
  ...
  'loc.calNote': '口径:本站库内在招岗(Job Bank 全国+ATS),非官方统计。',
  ...
  'quiz.sumNamed': '{n} 个命中「{s}」清单。', 'quiz.sumTeer': 'TEER {teer} —— 多数省提名通道的职业门槛卡在 TEER 0-3。',
  ...
  'acct.hero.login': '欢迎回来', 'acct.hero.reg': '免费注册,看每份工作对你的匹配度', 'acct.teaser': '注册后,每份工作都亮出你的匹配度:',
  ...
  'pnpdraws.reform.ON': '安省 2026-06 改制:旧的雇主担保类、硕博毕业生等 9 条通道已全部关闭(O.Reg 47/26),现行为 Workforce Priority 流。下列灰色抽选属已关闭的旧通道,仅作历史参考。',
  'ch.pnp.whySkilled': '凭什么:本岗是技能类职业(TEER {teer}),多数省提名的雇主类通道把职业门槛划在 TEER 0-3,本岗在门内。具体通道要求(语言、工资、居住)以该省官网为准。',
  'ch.pnp.whyOpen': '凭什么:{prov}现行规则不设职业清单,只列不受理的职业;本岗不在排除之列,所以 TEER {teer} 也算可提名。具体门槛(工资、全职永久 offer、语言)以该省官网为准。',
  'ch.pnp.qcWhy': '魁省不参加省提名(PNP),走自己的甄选体系(CSQ:Arrima 意向申报、PEQ 毕业生/工作经验通道),不适用本站的 PNP 判定与抽选历史。魁省动态见「资讯」页。',
  'ee.lastDraw': '(上次抽选 {d})', 'ee.dormantTip': '该 EE 类别最近一次抽选是 {d},已超过 12 个月未抽 —— 历史归属仍算,但现阶段不是活跃通道。',
  'eelist.federalNote': '类别抽选只是联邦抽选的一部分:现阶段联邦以 CEC(加拿大经验类)与法语类别为主。法语类别按语言能力判定、不按职业划分,故不在本表内。',
...
const en: Dict = {
  ...
  'fact.medianSrc': 'ESDC open data · same NOC × this province', 'fact.noMedian': 'No ESDC median data for this NOC × province', 'fact.medianPro': 'Local median comparison is a Pro feature', 'fact.vsNote': 'vs median = job annual ÷ local median − 1',
  ...
  'loc.calNote': "Based on this site's live index (Job Bank nationwide + ATS), not official statistics.",
  ...
  'quiz.lead': 'Three questions. Then we tell you: which provinces your occupation can immigrate through.',
  ...
  'quiz.q2': 'What do you do?', 'quiz.q2sub': 'Pick the closest one — every result below is calculated from it.', 'quiz.q2ph': 'Search a job title, e.g. cook, nurse aide',
  ...
  'quiz.sumNamed': '{n} hit the "{s}" list.', 'quiz.sumTeer': 'TEER {teer} — most PNP streams draw the line at TEER 0-3.',
  ...
  'acct.hero.login': 'Welcome back', 'acct.hero.reg': 'Sign up free — see your match for every job', 'acct.teaser': 'After you sign up, every job shows your match:',
  ...
  'pnpdraws.reform.ON': 'Ontario redesigned OINP in 2026-06: the 9 former streams (employer job offer, masters/PhD graduate, etc.) are all closed (O.Reg 47/26); the current one is the Workforce Priority stream. The greyed draws below are from closed streams — history only.',
  'ch.pnp.whySkilled': 'Why: this is a skilled occupation (TEER {teer}). Most PNP employer streams draw the occupation line at TEER 0-3, so it is inside. Stream-specific requirements (language, wage, residency) are on the province’s own site.',
  'ch.pnp.whyOpen': 'Why: {prov} currently publishes no occupation list — only occupations it will not accept. This one is not excluded, so TEER {teer} still counts as eligible. Thresholds (wage, permanent full-time offer, language) are on the province’s own site.',
  ...
  'ee.lastDraw': ' (last draw {d})', 'ee.dormantTip': 'Last draw for this EE category was {d} — over 12 months ago. The occupation still belongs to the category, but it is not an active pathway right now.',
  'eelist.federalNote': 'Category rounds are only part of Express Entry. Right now federal rounds are mostly CEC and French-language proficiency. The French category is judged on language ability, not occupation, so it is not in this table.',
...
const ko: Dict = {
  ...
  'fact.medianSrc': 'ESDC 공개 데이터 · 동일 NOC × 해당 주', 'fact.noMedian': '해당 NOC × 주의 ESDC 중위 데이터 없음', 'fact.medianPro': '현지 중위 임금 비교는 Pro 기능입니다', 'fact.vsNote': '중위 대비 = 직무 연봉 ÷ 현지 중위 − 1',
  ...
  'loc.calNote': '기준: 본 사이트 수집 공고(Job Bank 전국+ATS), 공식 통계 아님.',
  ...
  'quiz.lead': '세 가지 질문에 답하면, 당신의 직업이 어느 주에서 이민 경로가 있는지 바로 알려드립니다.',
  ...
  'quiz.q2': '어떤 일을 하시나요?', 'quiz.q2sub': '가장 가까운 것을 고르세요. 이후 모든 결과가 이를 기준으로 계산됩니다.', 'quiz.q2ph': '직무명 검색: 요리사, 요양보호사 등',
  ...
  'quiz.sumNamed': '{n}건이 「{s}」 목록에 해당합니다.', 'quiz.sumTeer': 'TEER {teer} — 대부분의 PNP 스트림 기준선은 TEER 0-3입니다.',
  'quiz.keep': '이전 답변 유지 →',
  ...
  'acct.hero.login': '다시 오신 것을 환영합니다', 'acct.hero.reg': '무료 가입 — 모든 공고의 매칭도를 확인하세요', 'acct.teaser': '가입하면 모든 공고에 매칭도가 표시됩니다:',
  ...
  'pnpdraws.reform.ON': '온타리오는 2026-06에 OINP를 개편했습니다. 기존 9개 스트림(고용주 오퍼, 석·박사 졸업생 등)은 모두 폐지(O.Reg 47/26)되었고 현재는 Workforce Priority 스트림입니다. 아래 회색 추첨은 폐지된 스트림의 이력입니다.',
  'ch.pnp.whySkilled': '근거: 이 직무는 숙련 직종(TEER {teer})입니다. 대부분의 PNP 고용주 스트림은 TEER 0-3을 기준선으로 하므로 범위 안입니다. 언어·임금·거주 등 세부 요건은 주 공식 사이트 기준입니다.',
  'ch.pnp.whyOpen': '근거: {prov}는 현재 직업 목록을 공개하지 않고 접수 제외 직종만 명시합니다. 이 직무는 제외 대상이 아니므로 TEER {teer}도 지명 가능으로 봅니다. 임금·정규직 오퍼·언어 등 기준은 주 공식 사이트 기준입니다.',
  ...
  'ee.lastDraw': ' (최근 추첨 {d})', 'ee.dormantTip': '이 EE 카테고리의 최근 추첨은 {d}로 12개월이 넘었습니다. 직업의 카테고리 소속은 유효하지만 현재 활성 경로는 아닙니다.',
  'eelist.federalNote': '카테고리 추첨은 Express Entry의 일부일 뿐입니다. 현재 연방 추첨은 주로 CEC와 프랑스어 능력 카테고리입니다. 프랑스어 카테고리는 직업이 아닌 언어 능력으로 판단하므로 본 표에 없습니다.',
