...
const zh: Dict = {
  ...
  'price.anchor': '参考:持牌顾问咨询通常 CA$150+/小时——先用 CA$19 把公开数据能回答的问题答掉。',
  ...
  'ps.title': '省提名自评打分', 'ps.sub': '填一次你的条件,按各省官方分值表分别算分,并与该省的官方分数线对照。', 'ps.official': '官方分值表', 'ps.you': '你的条件(填一次,各省按各自官方表折算)',
  ...
  'ps.noLine': '该省官方未公布分数线,只能看分值构成',
  'ps.range': '该省按通道分别设线,近 {n} 次 {lo}-{hi} 分;你的职业所属通道近期无抽选,不给差分结论',
  ...
  'ps.note': '按各省官方分值表自算,不是资格认定;官方以文件审核为准。',
  ...
  'price.pA': '我的通道报告', 'price.pA.d': '按你的档案算哪个省够线、差多少分:AI 顾问带档案分析、跨省对比、按档案筛出命中你的岗',
  'price.pB': '岗位盯梢', 'price.pC': '投递辅助',
  'price.pB.d': '你的职业与目标省一有新岗就到邮箱:匹配版邮件提醒、保存筛选 5 个位',
  'price.pC.d': '这家雇主值不值得投:多雇主并排对比、AI 顾问不限量追问',
...
const en: Dict = {
  ...
  'price.anchor': 'For reference: licensed-consultant sessions typically run CA$150+/hour — let CA$19 answer what public data can first.',
  ...
  'ps.title': 'PNP self-assessment score', 'ps.sub': 'Fill in your profile once; each province scores it with its own official points table and compares it to that province’s official line.', 'ps.official': 'official points table', 'ps.you': 'Your profile (fill once, scored by each province’s own table)',
  ...
  'ps.noLine': 'No official score line published — points breakdown only',
  'ps.range': 'Cut-offs are set per stream here: {lo}-{hi} across the last {n} draws. No recent draw for your occupation’s stream, so no gap is shown.',
  ...
  'ps.note': 'Self-calculated with each province’s official points table. Not an eligibility ruling — the province decides on documents.',
  ...
  'price.pA': 'My pathway report', 'price.pA.d': 'Which province you clear and by how many points: profile-aware AI advisor, province comparison, jobs filtered to your profile',
  'price.pB': 'Job watch', 'price.pC': 'Application help',
  'price.pB.d': 'New jobs in your occupation and target province, straight to your inbox: match alerts, 5 saved-search slots',
  'price.pC.d': 'Whether this employer is worth applying to: side-by-side employer comparison, unlimited advisor follow-ups',
...
const ko: Dict = {
  ...
  'price.anchor': '참고: 공인 컨설턴트 상담료는 보통 시간당 CA$150+ 이상입니다. CA$19로 공개 데이터로 확인할 수 있는 정보를 먼저 확인하세요.',
  ...
  'ps.title': '주정부 이민 자가 점수', 'ps.sub': '내 조건을 한 번 입력하면 각 주의 공식 점수표로 각각 계산하고 해당 주의 공식 기준선과 비교합니다.', 'ps.official': '공식 점수표', 'ps.you': '내 조건(한 번만 입력, 주별 공식표로 각각 환산)',
  ...
  'ps.noLine': '해당 주는 공식 점수 커트라인을 공개하지 않음 — 점수 구성만 표시',
  'ps.range': '이 주는 스트림별로 커트라인이 다릅니다(최근 {n}회 {lo}~{hi}점). 내 직종의 스트림은 최근 추첨이 없어 점수 차이는 표시하지 않습니다.',
  ...
  'ps.note': '각 주의 공식 점수표로 자가 계산. 자격 판정이 아니며 서류 심사는 해당 주가 합니다.',
  ...
  'price.pA': '내 이민 경로 리포트', 'price.pA.d': '어느 주에서 커트라인을 넘고 몇 점 부족한지: 프로필 기반 AI 상담, 주 간 비교, 프로필로 걸러낸 공고',
  'price.pB': '공고 모니터링', 'price.pC': '지원 도우미',
  'price.pB.d': '내 직종과 목표 주의 신규 공고를 이메일로: 매칭 알림, 저장 검색 5개',
  'price.pC.d': '이 고용주에 지원할 가치가 있는지: 고용주 비교, AI 상담 무제한 추가 질문',
