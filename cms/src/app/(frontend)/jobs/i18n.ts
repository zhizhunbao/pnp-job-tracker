...
const zh: Dict = {
  ...
  'gr.sponsorPill': '担保记录 {g}/5',   // #131 人话化:裸「担保 3/5」Frank 实测看不懂
  // #132 档名人话化(Frank「X/5 用户能看懂吗」):全站主显档名,数字降级灰小注
  ...
  'gr.sponsorTip': '该雇主的外劳担保记录强度(1-5 档,依据官方 LMIA 获批历史与新近度);点公司名看明细',
  ...
  'mm.v.notTarget': '不在目标省', 'mm.v.qc': '魁省独立体系', 'mm.v.named': '省点名招', 'mm.v.excluded': '清单排除', 'mm.v.generic': '符合{prov}通用粗筛', 'mm.v.provNone': '不符合{prov}通用粗筛',
  ...
  'fact.scoreNote': '各维度独立 1-5 档(不加权不合成),与列表「通道」列同源;粗筛信号,非资格认定。档位对所有人相同——个人化结论请用「我的匹配」(需建档)', 'score.base': '基准', 'score.indemand': '紧缺大类', 'score.low': '省提名清单命中', 'score.direct': '第一方雇主', 'score.exp': '经验', 'score.prov': '省份(非 ON)', 'score.total': '合计', 'score.stored': '入库',
...
const en: Dict = {
  ...
  'gr.sponsorPill': 'Sponsor record {g}/5',
  ...
  'gr.sponsorTip': 'Strength of this employer’s foreign-worker sponsorship record (1-5, from official LMIA approvals and recency); click the company name for details',
  ...
  'mm.v.notTarget': 'Not your target province', 'mm.v.qc': 'Quebec has its own system', 'mm.v.named': 'Named by province', 'mm.v.excluded': 'Excluded by list', 'mm.v.generic': 'Passes {prov} pre-screen', 'mm.v.provNone': 'Fails {prov} pre-screen',
...
const ko: Dict = {
  ...
  'gr.sponsorTip': '이 고용주의 외국인 근로자 스폰서 기록 강도(1-5, 공식 LMIA 승인 이력·최신성 기준); 회사명을 클릭하면 상세',
  ...
  'mm.v.notTarget': '목표 주 아님', 'mm.v.qc': '퀘벡 독자 체계', 'mm.v.named': '주 지명 수요', 'mm.v.excluded': '리스트 제외', 'mm.v.generic': '{prov} 일반 선별 통과', 'mm.v.provNone': '{prov} 일반 선별 미달',
  ...
  'fact.scoreNote': '각 차원은 독립적으로 1-5 등급(가중치·합산 없음), 목록 「이민 경로」 열과 동일한 소스; 자격 판정이 아닌 대략적인 신호입니다. 등급은 모든 사용자에게 동일하며, 개인화된 분석은 「나의 매칭」(프로필 필요)을 이용하세요.', 'score.base': '기준', 'score.indemand': '수요 직종 그룹', 'score.low': 'PNP 직종 목록 포함', 'score.direct': '직접 고용주', 'score.exp': '경력', 'score.prov': '주(ON 제외)', 'score.total': '합계', 'score.stored': '저장됨',
