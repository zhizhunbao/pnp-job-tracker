...
const zh: Dict = {
  ...
  'case.back': '返回 PR 评估', 'case.theQuestion': '用户原话', 'case.official': '官方原文', 'case.asOf': '核对于 {d}',
  'case.askedTitle': '{prov}:这条路径排在什么位置',
  'case.askedLead': '按官方规则判定,{prov} 属于「{tier}」这一档。在下列可走路径中,最快的一条为「{fastest}」。该省的具体门槛与扣分如下。',
  'case.othersTitle': '可走的路径,由易到难',
  'case.othersLead': '排序依据是「拿到雇主 offer 之后还需积累多久」;同一档内按官方注册表原序排列,本站不另行加权。',
  'case.blockedTitle': '目前走不通的路径',
  'case.blockedLead': '以下路径存在攒时间也补不齐的差距,官方原句一并列出。',
  'case.blockedTag': '走不通',
  'case.ops.spots': '{period} 名额 {total},已提名 {used},剩 {left}',
  'case.ops.alloc': '{period} 名额 {n}', 'case.ops.pool': '池中 {n} 人',
  'case.ops.approved': '{period} 提名阶段批准 {pct}%(提名 {ok},拒签 {no})',
  'case.claimLead': '关于「概率」:官方公布的最接近数字是提名阶段的批准率 {pct}%({period} 提名 {ok} 件、拒签 {no} 件)。但它衡量的是收到邀请之后的一段;同期该省仅发出 {invited} 份邀请,而你尚未进入候选池,且已被倒扣分。提名之后仍需联邦审批,提名不等于永居。',
  'case.tier0': '无需积累', 'case.tier1': '约 3-6 个月', 'case.tier2': '约 12 个月', 'case.tier3': '约 24 个月',
  'case.firstStepTitle': '第一步:先找肯带训的雇主',
  'case.firstStepLead': '上述路径均以雇主 offer 为前提。全国现有 {n} 个该职业岗位由官方标注为提供带训或不要求经验,按省分布如下。',
  'case.mineTitle': '你的条件与此不同',
  'case.mineLead': '以上判定基于该案例的完整档案。语言成绩、工作经验与目标省份任一不同,排序都会改变。',
...
const en: Dict = {
  ...
  'case.back': 'Back to PR assessment', 'case.theQuestion': 'In their words', 'case.official': 'Official text', 'case.asOf': 'as of {d}',
  'case.askedTitle': '{prov}: where this pathway actually ranks',
  'case.askedLead': 'Assessed against the official rules, {prov} falls into the "{tier}" band. The fastest pathway available below is "{fastest}". Its specific requirements and deductions are set out here.',
  'case.othersTitle': 'Viable pathways, easiest first',
  'case.othersLead': 'Ranking reflects how long you must still accumulate after securing an employer offer. Within a band the official registry order is kept; Offer2PR applies no further weighting.',
  'case.blockedTitle': 'Pathways currently closed',
  'case.blockedLead': 'The gaps below cannot be closed by accumulating time. The official wording is quoted with each.',
  'case.blockedTag': 'Closed',
  'case.ops.spots': '{period}: {total} spots, {used} nominated, {left} left',
  'case.ops.alloc': '{period}: {n} spots', 'case.ops.pool': '{n} in the pool',
  'case.ops.approved': '{period} nomination-stage approval {pct}% ({ok} nominated, {no} refused)',
  'case.claimLead': 'On the "probability": the closest published figure is the nomination-stage approval rate of {pct}% ({ok} nominated against {no} refused, {period}). It measures the stage after an invitation. Over the same period the province issued only {invited} invitations, and you are not yet in the pool and already carry a deduction. A nomination still requires federal approval; it is not permanent residence.',
  ...
  'case.firstStepLead': 'Every pathway above presumes an employer offer. {n} openings in this occupation are officially flagged as training-provided or no-experience; the split by province follows.',
  'case.mineTitle': 'Your details will differ',
  'case.mineLead': 'This assessment uses the full profile of the case. Any difference in language score, work experience or target province changes the ranking.',
...
const ko: Dict = {
  ...
  'case.back': 'PR 평가로 돌아가기', 'case.theQuestion': '사용자 원문', 'case.official': '공식 원문', 'case.asOf': '{d} 확인',
  'case.askedTitle': '{prov}: 이 경로의 실제 순위',
  'case.askedLead': '공식 규정으로 판정하면 {prov}은(는) "{tier}" 구간에 속합니다. 아래 가능한 경로 중 가장 빠른 것은 "{fastest}"입니다. 해당 주의 요건과 감점은 다음과 같습니다.',
  'case.othersTitle': '가능한 경로, 쉬운 순서',
  'case.othersLead': '정렬 기준은 고용주 오퍼 확보 후 추가로 필요한 기간입니다. 동일 구간 내에서는 공식 등록 순서를 유지하며 별도 가중치를 적용하지 않습니다.',
  'case.blockedTitle': '현재 진행할 수 없는 경로',
  'case.blockedLead': '아래 격차는 시간을 쌓아도 메울 수 없습니다. 공식 문구를 함께 제시합니다.',
  'case.blockedTag': '불가',
  'case.ops.spots': '{period} 정원 {total}, 지명 {used}, 잔여 {left}',
  'case.ops.alloc': '{period} 정원 {n}', 'case.ops.pool': '풀 {n}명',
  'case.ops.approved': '{period} 지명 단계 승인률 {pct}%(지명 {ok}, 거절 {no})',
  'case.claimLead': '"확률"에 대하여: 공개된 가장 가까운 수치는 지명 단계 승인률 {pct}%({period} 지명 {ok}건, 거절 {no}건)입니다. 이는 초청을 받은 이후 단계를 측정합니다. 같은 기간 초청은 {invited}건에 불과했고, 귀하는 아직 풀에 들어가지 않았으며 이미 감점을 안고 있습니다. 지명 후에도 연방 심사가 남아 있으며 지명은 영주권이 아닙니다.',
  'case.tier0': '대기 없음', 'case.tier1': '약 3-6개월', 'case.tier2': '약 12개월', 'case.tier3': '약 24개월',
  'case.firstStepTitle': '첫 단계: 교육을 제공하는 고용주 찾기',
  'case.firstStepLead': '위 경로는 모두 고용주 오퍼를 전제로 합니다. 해당 직종에서 교육 제공 또는 경력 무관으로 표시된 공고는 전국 {n}건이며 주별 분포는 다음과 같습니다.',
  'case.mineTitle': '귀하의 조건은 다릅니다',
  'case.mineLead': '위 판정은 해당 사례의 전체 프로필 기준입니다. 언어 점수·경력·희망 주가 하나라도 다르면 순위가 달라집니다.',
