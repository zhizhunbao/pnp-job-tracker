...
const zh: Dict = {
  ...
  'case.claim.title': '关于「概率」',
  'case.claim.rate': '官方公布的最接近数字是提名阶段批准率 {pct}%,{period} 提名 {ok} 件、拒签 {no} 件',
  'case.claim.stage': '它衡量的是收到邀请之后的一段',
  'case.claim.invites': '同期该省仅发出 {invited} 份邀请',
  'case.claim.pool': '你尚未进入候选池,且已被倒扣分',
  'case.claim.federal': '提名之后仍需联邦审批,提名不等于永居',
...
const en: Dict = {
  ...
  'case.claim.title': 'On the "probability"',
  'case.claim.rate': 'The closest published figure is the nomination-stage approval rate of {pct}%, {ok} nominated against {no} refused, {period}',
  'case.claim.stage': 'It measures the stage after an invitation',
  'case.claim.invites': 'Over the same period the province issued only {invited} invitations',
  'case.claim.pool': 'You are not yet in the pool and already carry a deduction',
  'case.claim.federal': 'A nomination still requires federal approval, it is not permanent residence',
...
const ko: Dict = {
  ...
  'case.claim.title': '"확률"에 대하여',
  'case.claim.rate': '공개된 가장 가까운 수치는 지명 단계 승인률 {pct}%, {period} 지명 {ok}건 거절 {no}건',
  'case.claim.stage': '이는 초청 이후 단계를 측정함',
  'case.claim.invites': '같은 기간 초청은 {invited}건에 불과',
  'case.claim.pool': '아직 풀에 들어가지 않았고 이미 감점을 안고 있음',
  'case.claim.federal': '지명 후에도 연방 심사가 남으며 지명은 영주권이 아님',
