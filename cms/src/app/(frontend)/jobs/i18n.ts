...
const zh: Dict = {
  ...
  'dp.compNoteShort': '名额竞争 = 学签+工签存量(IRCC,截至 {m},不含访客)÷ 省提名名额(本站人工核对);新发学签只看趋势,不参与比值;PE 官方只发布含 AIP 的合并名额,无单列数不入表;本站更新 {d}',
...
const en: Dict = {
  ...
  'dp.compNoteShort': 'Competition = study + work permit holders (IRCC, as of {m}, visitors excluded) ÷ nomination allocation (our verified table); new study permits show trend only, not in the ratio; PEI publishes only a combined PNP+AIP allocation and is omitted; updated {d}',
...
const ko: Dict = {
  ...
  'dp.compNoteShort': '경쟁 = 학생+취업비자 보유자(IRCC, {m} 기준, 방문자 제외) ÷ 지명 쿼터(본 사이트 검증); 신규 학생비자는 추세 참고용, 비율 미포함; PEI는 PNP+AIP 합산 쿼터만 발표해 표에서 제외; 본 사이트 갱신 {d}',
