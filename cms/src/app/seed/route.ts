...
export async function GET(req: Request) {
  ...
  const dims: [string, string, string[], (r: any) => Row][] = [
    ...
    ['pnp_score_factors', 'pnp_score_factors', ['province', 'system', 'factor', 'kind', 'seq', 'label', 'points', 'xor_prev', 'rule', 'factor_max', 'max_total', 'guide_effective', 'url', 'fetched'],
      (r) => ({ province: r.province, system: r.system, factor: r.factor, kind: r.kind, seq: r.seq, label: r.label, points: r.points, xor_prev: r.xorPrev, rule: r.rule, factor_max: r.factorMax, max_total: r.maxTotal, guide_effective: r.guideEffective, url: r.url, fetched: r.fetched })],
