...
export const DISEASE_CARDS: DiseaseCard[] = [
  ...
  { id: 'K03', layer: 'synth', status: 'open',
    symptom: '省名串台:「新不伦瑞克省 NL NLPNP International Graduate」;AB+BC 时「下一步」指向 BC(实测两例)',
    guard: 'eval 检查器(答复省名集合 ⊆ facts∪问句省名集合)' },
  { id: 'K04', layer: 'synth', status: 'open',
    symptom: '「要求这份工作至少给到中位」压成「申请人年薪门槛」——口径压扁(chat_logs #36/#37 同串实测)',
    guard: 'eval 检查器(申请人+年薪+门槛连写)+ 报告全文人审' },
  { id: 'K05', layer: 'slots', status: 'open',
    symptom: '「手里的 PGWP 还有效」被抽成 claims 且没触发裁决(08-06 实测,抽槽方差)',
    guard: 'eval 检查器(claims 里出现第一人称自述句)' },
  ...
  { id: 'K08', layer: 'slots', status: 'partial',
    symptom: '「33102 (PSW)」裸码开头被抽槽模型抄成 31102(chat_logs #94 实录;08-09 基线批跑 R18 复发,答复两码混说)——模板侧已治(补 NOC 前缀),用户裸码输入侧未治',
    guard: '模板侧=chatExamples.int.spec 回归;用户侧=corpus R18 expectNocFinal 在守(根治候选=literalNoc 裸码扩围带库验证,STATUS 已记账)' },
