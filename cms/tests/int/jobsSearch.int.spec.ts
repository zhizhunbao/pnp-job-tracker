...
describe('buildJobsWhere:q 多词', () => {
  ...
  it('两词:两组条件用 AND 串,各自一个占位符', () => {
    const w = buildJobsWhere({ q: 'cook toronto' })
  ...
  it('短词(≤2 字)才带省码列:trgm 索引对 <3 字退化,长词走这列是死分支', () => {
    expect(buildJobsWhere({ q: 'ON' }).sql).toContain('j.province ILIKE $1')
    expect(buildJobsWhere({ q: 'cook' }).sql).not.toContain('j.province ILIKE')
  ...
  it('公司名预查按词各一组:第 i 组喂给第 i 个词', () => {
    const w = buildJobsWhere({ q: 'cook toronto', qCompanyIds: [[7, 8], []] })
