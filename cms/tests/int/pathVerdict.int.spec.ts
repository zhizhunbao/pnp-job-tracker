...
import type { PathLeverOpts } from '@/lib/ruling/server'
...
import {
  scoreProvince as pointsScore,
  type EeGridRow, type ScoreFactor, type ScoreOverride, type SelfProfile,
} from '@/lib/points'
...
describe('红线不变量', () => {
  ...
  it('NL 指定雇主是 supporting fact:650 家里 4 家申报过 72310(2026-08-31 官方名录 639 → 645,pnp 域 C01 金标同步;2026-09-16 645 → 650:nl_employers 步 09-15 挂进 pnp 自动刷新链,官方名录新增 5 家)', () => {
    const nl = byKey(list, 'NL-intl-grad')
    const fact = nl.reasons.find((r) => /指定雇主/.test(r.text))!
    expect(fact.text).toContain('650')
