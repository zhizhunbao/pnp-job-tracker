...
const F = (p: Partial<PoolFilters> = {}): PoolFilters =>
  ({ group: '', prov: '', city: '', district: '', broad: '', sector: '', program: '', noc: '', entry: false, lmia: false, q: '', sort: 'star', dir: 'desc', page: 0, ...p })
...
describe('行构造器', () => {
  it('numeric 列的字符串收成数,可空数值保 null,jsonb 清单成数组', () => {
    const r = toPoolRow({
      key: 'shopify', slug: 'shopify', name: 'Shopify', industry: 'IT', sector: null, province: 'ON', city: 'Ottawa', district: 'Kanata', website: 'https://www.shopify.com', x_status: null, x_alias_zh: null, x_alias_ko: null, x_trans_v: null, city_zh: '渥太华', city_ko: null, trans_v: 1,
  ...
  it('三源独有雇主:slug / 行业 / 季度 为 null 不折空串以外的东西', () => {
    const r = toPoolRow({
      key: 'n:acme', slug: null, name: 'Acme', industry: null, sector: 'municipal', province: 'NB', city: '', district: null, website: 'shopify.com', x_status: 'done', x_alias_zh: '艾克米', x_alias_ko: null, x_trans_v: 1, city_zh: null, city_ko: null, trans_v: null, locations: null, designated: true,
  ...
  it('默认(没组没词)= 全组一家一行:DISTINCT ON 查询,参数全空;省下拉照给;同参第二次走缓存不再查', async () => {
    ...
    const { pool, seen } = fakePool((sql) => {
      ...
      if (sql.includes('jsonb_array_elements_text')) {
        return { rows: [{ broad: 'IT', broad_en: 'Tech', broad_ko: null }] }
      }
      return { rows: [] }
    })
    const p = await loadEmployerPage({ db: pool, filters: F(), pageSize: 50 })
    expect(p.broads).toEqual([{ key: 'IT', en: 'Tech', ko: '' }])
