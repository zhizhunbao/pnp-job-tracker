...
type Seq = { name: string; first: string; follow: string; profileOnFollow: Partial<Profile> }
...
describe.skipIf(!LIVE)('追问:上一轮的内容记不记得住', () => {
  for (const s of SEQS) {
    it(s.name, async () => {
      ...
      const t1 = await consult({ db, text: s.first, lang: 'zh', profile: EMPTY_PROFILE, history: [], onStep: null, onDelta: null, loadVerdict: loadVerdictTables, judgeVerdict: judgeWithRuling })
      console.log(`\n【${s.name}】`)
      ...
      const t2 = await consult({ db, text: s.follow, lang: 'zh', profile: { ...EMPTY_PROFILE, ...s.profileOnFollow }, history, onStep: null, onDelta: null, loadVerdict: loadVerdictTables, judgeVerdict: judgeWithRuling })
