...
- **上线当天实撞的竞态(已修)**:工人 14:04 把 Cotech 的官网 / 总部写进库,14:06 灌库拿的是 13:58 汇装的 mart,又盖回死官网和空总部。
  修法:工人每次写公司表都把 `site_checked_at` 盖成现在;灌库的 companies upsert 多一组「刚核对过就不盖」列(官网 / 官网来路 / 总部五格 / 核对时刻),
  库里那一行 3 小时内核对过的保库里的值(一轮汇装一小时,过了这段 mart 早已带上同样的新值)。
  同源的另一处:company 域照旧官网写的五节简介还在 briefs 里,汇装会把换对的简介盖回去 → mart `is_stale_brief`:富化缓存记了 `replaces` 且简介出处里有那台旧主机的不再喂。
- explore 域默认链改成先消费译名、后落清单:清单接口出问题不该挡住译名(cms 还没换版时实撞 404)。
- 生产验收(容器里的工人,Bing 档):Cotech `nouveau.cotech.ca` → `cotech.ca`,总部 412 route de la Station, St-Éloi, QC,约 290 秒;
  Armoires de cuisine Bernier `cuisinesbernier.ca` → `armoiresbernier.com`,总部 Lévis, QC、成立 1965,约 260 秒;卡上进度逐步变化、内容自动出现。
- **生产验收里照出来的三处找官网的洞(已修,City of Coquitlam 一家全撞上)**:① 真官网 coquitlam.ca 搜索排第一,但 `<title>` 在首页第 6172 个字符(前面一串统计脚本),
  护栏只嗅前 4000 字符、嗅不到标题就跳过,排第三的 visitcoquitlam.ca 反而中了 → `TITLE_SNIFF_LEN` 放到 20000;点开路径护栏没过的再用有头浏览器开首页读标题(`hot_site_of_links`,拦 httpx 的站也能核)。
  ② Wikidata 登记的官网就是那个死域名 city.coquitlam.bc.ca,阶梯又把它选了回来 → 候选域名先过 `host_resolves`(解析不了的不要;函数收进 fetch 叶,sites 判死站也用它)。
  ③ 阶梯只避开「现在那个官网」,换过一次以后原来的死官网不在避开之列 → 避开集 = 现在的 + 这家以前被顶掉过的(`replaces` 只记头一回的:那才是来源侧带的旧官网,mart 比的是它)。
  修完重查:City of Coquitlam → coquitlam.ca。
