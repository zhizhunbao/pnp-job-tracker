...
待拍:无。批次纪律一批一 session:批一数据层(2026-09-04 已上生产)→ 批二 guide 域(2026-09-05 已提交:`lib/guide` 五件 + 两门、`/api/guide` 与 `/api/guide/email` 两壳、23 条纯函数回归、真模型真库探针 8/8 对)→ 批三挂件换装(切消费者到 `/api/guide`,再整删 consult 目录、评测批、eslint 的 consult 特批行)。
