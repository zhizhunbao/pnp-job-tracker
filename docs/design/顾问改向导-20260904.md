...
- **加**:开场白一条(线程首项,纯文案)。闲聊类(打招呼、「有什么功能」「怎么使用」)不显示模型那句,出固定一句「站上有这些,点一个过去」+ 主要页面八张卡横向流式(2026-09-05 Frank「???? 这是回答了什么」);胶囊与卡一律按文字包裹(Frank「这个按钮是不是太宽了」)。
...
## 9. 答题批(2026-09-06 Frank 拍板,推翻 §6「不答题」一条)

> 起因:Frank 实拍四条胶囊里「安省提名要什么条件」「哪些雇主拿过 LMIA」两条都回「这个站上还没有」——「这个能参考下 gpt 的吗?现在看着几乎是没法用。」/fe 评估后多选拍板:**站内数据先答再记** + **切 Anthropic Haiku** + 顺带修 PTE 口语带错页、URL 过滤项目名(429 提示已有 `chat.err.limit`,未动)。「全 GPT 化自由作答」「维持只带路」「砍挂件」三项未勾。

- **问题类多一格 `topic`**(pnp / lmia / jobs / null,只在 kind=question 时填):模型只说「这是哪一类问题」,事实一律从库取 —— `pnp` → `pnp_requirements` 按省前 24 条;`lmia` → 拿过 LMIA 且在招的雇主前 8 家(按 TEER 0-3 岗位数排,裸 LMIA 总量不入排序);`jobs` → 某职业码在招量 / 可提名量 / 中位薪资 + 省分布前 5。省提名没省码、职业没职业码 = 事实空 = 照旧「记下」。
- **第二次调用只组织不添**(`ANSWER_ROLE`):最多六行、每行一条事实、数字名字单位原样、不判资格不承诺不贴 URL,事实不够答就回空串;空串在代码里退回「记下」+ 留邮箱卡。答上来的 kind 仍记 question(后台分组不变),say 非空、dest 落题目对应页(pnp → PR 决策页,lmia → 雇主榜,jobs → 职位板),挂同一张「打开 X」卡,不出邮箱卡。
- **模型通道钉死 Anthropic**(`guide/constants.ts` PROVIDER):friend 网关实测 2.5~9.8 秒、同句不同判(「哪些雇主拿过 LMIA」Frank 实拍判 question、我实测判 nav)、整段提示词缓存粘住。
- **PTE 子项带题型与分部**(`prompts.ts` SUB_DESC):「口语题」此前落 `/pte/sst`(听力的 Summarize Spoken Text);目录行现在写 `ra (Read Aloud, Speaking)`,规则说名分部取该分部第一项。
- **q 过滤项目名**(`PROGRAMME_Q_RE`):实拍带路 URL 带了 `?q=LMIA`,提示词禁不住,代码侧整格丢弃。
- **追问类 `reply`**(2026-09-07 Frank 实拍「什么意思」「答非所问」两次弹出站内地图:「你得正常跟用户沟通啊」):接着上一轮说的话单独一类,模型看着历史用一两句话回应(复述上一句指的是哪页、或认错并问他想看什么),挂件只显示这一两句,不弹地图不出卡;闲聊类仍只收「整句无请求」的打招呼与「有什么功能」。
- 数据侧:直连生产库拉 asks 表被本 session 分类器拦下,改版前后的分类分布没拿到;上线后看 asks 表 kind=question 且 say 非空的占比即「答上来的比例」。
