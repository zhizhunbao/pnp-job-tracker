...
- [x] `https://pnp-cms.onrender.com/jobs` 公网 200（15MB 云库数据）；`/seed` 无 token 401。⚠️ 现为 **Free 档**（闲置休眠 50s 冷启动）——公测宣传前升 Starter；正式域名待定（品牌名决策挂起）。
- [~] 完整链路自转：手动全链已通（upload_mart→Storage→seed 远程→云库→前端）；**$7 起步档=家里 build 容器自动推云端**（compose 已接 SEED_URL/SEED_TOKEN/SUPABASE_*，等下轮验证）。Render worker 后置（M4 前升级全托管时启用）。
- [x] seed 双模式本地回退回归通过（jobs 12019 基线全等）。⚠️ **注意：cms/.env 的 DATABASE_URI 现指 Supabase = 本地 dev 直连生产库**（`seed?reset=1` 会重灌生产！）——日常开发建议切回注释掉的本地库行,要看生产才切云。
- [x] Supabase：schema 已推（pooler 5432 会话模式）；bucket `mart` 私有（API 建）；12 表上传成功（jobs.json 28MB<50MB 上限）。
- [ ] worker 持久盘（Render worker 启用时适用;当前 ETL 状态在家里机器,天然持久）。
