...
**8 个提交已于 2026-08-18 03:50 推送**(`b2be87da..4d2d9e0c`),GitHub 上确认到位。
推前在本地跑过 `npm run build`(✓ Compiled successfully)。

🔴 **但生产没换版**:推后 20 分钟,生产 CSS 包仍是 19,736 字节、零个本轮标记
(连 08-17 那轮就有的 `factRow` 都没有)—— 生产停在 `main.css` 637 行那个版本。
**接手第一件事:确认生产到底换没换版**(拉 `https://offer2pr.com/` 的 CSS 包 grep `cardMd`),
没换就去 Render 面板看是排队、构建失败,还是 auto-deploy 没开(有构建分钟耗尽的先例)。
**别默认线上跑的是仓库里这份代码。**
