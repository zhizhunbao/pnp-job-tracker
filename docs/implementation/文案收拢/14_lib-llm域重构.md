...
> 🔴 **订正(2026-08-19 当日晚,[15 号 §1](15_lib-chat换编排走pi.md#1--账实测翻盘不花钱)):
> 「朋友的网关不透传 tools」这句已作废** —— 对 `/v1/chat/completions` 实测回的是
> `finish_reason: "tool_calls"` 加合法的 `tool_calls`,四轮多工具回灌一次没掉链子。
> 那次实测多半打的是旧链 `/api/chat`。
> **本节「不合并」的结论不受影响**(它靠的是「有没有重复」与「按对话形状切边界」,不是靠这一句);
> 受影响的是「`lib/agent` 换不了后端」那条 —— 它换得了,而且换过去就不花钱。
