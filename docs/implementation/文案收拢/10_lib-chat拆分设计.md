...
> **状态:已落地** —— Frank「都行」= 按 §9 三个推荐默认值执行,提交 `43e29f23`,生产已换版。
> §1-§9 是**动手前**写的设计,原样保留;**实际怎么落的、和设计差在哪** 见 §10。
...
## 10 · 落地记录(2026-08-18,提交 `43e29f23`)

### 实际形状:17 文件 4879 行,**有向无环 8 层**(按真实 import 图重算,不是段级预测)

```
1 tools(1143)
2 types(75) · wording(157)
3 federal(167) · normalize(48) · steps(50) · log(75)
4 slots(429) · traces(173)
5 answer(492) · facts(375) · guards(520)
6 cards(466) · followups(117) · stream(154)
7 orchestrate(410)
8 index(28)
```

桶 **23 个**(7 值 + 16 类型),测试直接点文件,`tests/**` 在边界闸上有一个写明理由的口子。
`lib/chat` 同时加进带桶模块清单(`BARRELS`)。

### 🔴 三道闸各抓到不同的东西 —— 一道都不能省

| 闸 | 抓到什么 |
|---|---|
| **tsc** | ① 展开语法 `...planFacts(…)` 长得像成员访问,标识符扫描整个漏掉 → 4 个「只以展开形式被调用」的名字没生成 import;② `ChatError` 是**类**,不能跟着「契约」段进 `types.ts`(那会毁掉「一个运行时值都没有」,而 `i18n/chat` 的反向 import 靠的就是它);③ 已经指向新目录的相对路径(`./chat/tools`)不能再整体下移一层 |
| **eslint** | 标识符扫描**不剥字符串** —— 提示词正文里写着 `lookupJobs` 这样的词,于是生成了 **18 个假 import**。照它的 `no-unused-vars` 报告逐条摘掉 |
| **vitest** | **另外两道都没抓到的那个**(见下) |

### 🔴 tsc 全程绿、行为却被改坏的那一个

`stripMd` 是**多行链式箭头**(没有大括号):

```ts
export const stripMd = (s: string): string => s
  .replace(/\*\*/g, '')      // ← 按「大括号配平」判块边界时,这些尾巴不算它的
  .replace(/`/g, '')
  …
```

按大括号判边界 → **只搬走了第一行**,`stripMd` 变成恒等函数;而留在原地的 `.replace(…)` 尾巴
**接到了上一条语句 `zhOnly` 上** —— 于是两个函数同时被改坏,**tsc 全程绿**(链式接在一个 string 上,类型完全合法)。

3 个用例当场红(「markdown 记号一个都不许见客」那组)。**这就是「按用例全名前后对拍」的回报** ——
只看 passed 数字或只看 tsc,这个会静默上线。

### 段级预测 vs 实际:少算了一个环

设计 §3 预测「9 个名字下沉 wording 后无环」。实际还剩一个 **`guards ↔ wording`**:
`UNIT_WORDS` / `NOT_PROPER` 两张词表住在 guards 段里,但它们是**词表**不是闸。
一并下沉 wording.ts,环消失。

**段级模型只能预测到「段」的粒度** —— 同一段里混着两种东西时它看不出来。
所以拆完必须按**真实 import 图**再查一次环,不能拿设计时的预测交差。

### 注释里的旧文件名 20 处一并改

带点的引用按**那个名字现在住哪**改,不是一律改成 `lib/chat`:
`chatOrchestrate.factSheet` → `lib/chat/stream.factSheet`、`chatOrchestrate.citeFacts` → `lib/chat/followups.citeFacts`。
剩下没改的只有**测试文件名**的引用(`chatOrchestrate.int.spec.ts` 这些文件本身还叫这个名)。
...
|---|---|---|
| `tsc --noEmit` | 0 | **0** |
| eslint | 558 problems / 1 error | **558 / 1**(逐个数字相同) |
| vitest | 699 / 694 过 / 5 败 | **699 / 694 / 5**,按用例全名比对新增 0、变绿 0 |
| `npm run build` | — | **✓**(本地先跑,别拿 Render 的构建分钟当编译器) |
| 生产 | — | `/api/version` → `43e29f23`;`/` `/plan/pr` `/employers` 200;`GET /api/chat` **405**(不是 500 —— 路由模块真的加载起来了) |

### 还没做的

- **测试文件名没跟着改**(`chatOrchestrate.int.spec.ts` 等):改名是另一件事,且会让 git 历史更难追。
- `tests/eval/*.eval.spec.ts` 只由 tsc 保证编译得过,**没跑**(它们打真模型)。
