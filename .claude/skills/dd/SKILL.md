...
读工作台现状(`docs/功能盈利点检查.md`)→ 实施上轮待整改项(代码侧自动做,用户手动项列出)→ 无截图体检(捆绑脚本)→ **人眼走一遍转化链路**(浏览器窗格,第 2.5 步)→ 统一检查(闭环 ✅ + 新问题追加编号 + **每个数据面过内容层六问**:列口径读到 ETL/列非空率/站内重复/形态对照 JobsTable/口径时效/内容置位(这列这文案放这儿合不合适),见 iterate-audit 第 3 步)→ 更新文档轮次 → 输出本轮报告。
