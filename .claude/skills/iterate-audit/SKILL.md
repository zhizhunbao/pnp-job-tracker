...
3. **内容层五问(每个数据面/表格必过,2026-08-08 Frank 四连击后定死)**——体检脚本和截图只能查「渲染对不对」,查不出「该不该存在」,这五问必须逐块人肉过:
   - ① **列口径**:每根列的口径是什么、数据哪来?**读到 ETL 规则为止**,别只看列名(担保档教训:档名「常年办」与规则背离,AIP 兜底渲「办过 LMIA」假话);
   - ② **列覆盖率**:每根列非空率数一遍(DOM 一行 JS 就能数),稀疏列=数据缺口或该撤列(中文名 15/100 教训:懒查询字段被放进批量展示列);
   - ③ **重复**:这一块与站内哪里重复?同数据源同列组出现两处=其中一处该压缩(把脉三分表≈货架三视图复刻教训);
   - ④ **形态站规**:控件/搜索/筛选形态对照 JobsTable 站规,自造布局即违规(搜索区跑偏教训);
   - ⑤ **口径时效**:数据窗口还匹配政策现实吗?(「近两年」掺 2024 宽松旧常态教训——LMIA 件数两年 -61%,旧口径无预测力);
   - ⑥ **内容置位**:这个内容放在**这个位置**是否有意义、是否合适?列要服务本视图人群的决策,文案要服务本页动作——自身正确但放错地方的,照样挪或删(担保档是 LMIA 维度却挂在 AIP/PNP 视图、?noc= 筛了职业「在招」列仍显全量数教训);
   - 附:**中文态两查**——该翻译的要有中文;有中文的必须简体(Wikidata zh 标签常混 zh-TW,zhconv 扫一遍)。
