...
**只动了某几个模块时加 `--only`**(2026-08-02 加):`--only jobs,pricing`,模块名 `jobs / pricing / stats / rankings / news / pathways / plan / account`(定义见脚本顶部 `MODULES`),不传=全跑。单模块 1-2 分钟,全量 ~7 分钟;`--only` 没选到登录态页面时测试号段整段跳过。**大改版或隔了几轮没全量,仍跑全量**——模块过滤省的是时间,不是覆盖率。
...
### 第 2.5 步 · 人眼走一遍转化链路(每轮必做,~5 分钟)

脚本量的是 DOM,量不出「点下去才知道是死路」「等得心慌」「挤在一起难看」。所以每轮**真开浏览器走一遍**:用会话内的浏览器窗格(`mcp__Claude_Browser__`,`preview_start {url: "https://offer2pr.com"}` + `resize_window {preset: "mobile"}`),手机视口走这一条固定链路,**不漫游**:

> 首页 → /jobs 列表(滚两屏 + 开一次「移民价值」弹框)→ 点进一个职位详情 → /pricing → 点「购买 30 天」走到 Stripe 页为止(**只看不付**)

桌面视口只在本轮动过桌面版式时补走。用测试号时自己登(`credentials.local.json`);**只有需要 Frank 的真实登录态**(Stripe 后台、Umami 面板、Render)才切 `mcp__claude-in-chrome__`,且要先问过他。

只记脚本抓不到的三类,别重复体检脚本已覆盖的裸词/折行/死链:

1. **断链**:点了没反应、跳错页、返回丢筛选状态;
2. **等待感**:哪一步白屏或骨架屏超过 3 秒;
3. **视觉**:留白挤、层级乱、字号跳、图文错位。

`--only` 模式下只走该模块相关的那一段链路。发现的一律记进清单等下一轮,**不当场改**(同第 3 步纪律)。

### 第 3 步 · 统一检查

把三路输入合到一起判:体检命中清单(第 2 步)、人眼浏览所见(第 2.5 步)、截图(逐张 Read,全部不抽查)。两件事同时做:
...
- 轮次 N → N+1,标题与日期更新;**上一轮的「当前轮次」段落先落成独立文件 `docs/整改轮次/第NN轮.md`,并在索引 `docs/整改轮次存档.md` 表里加一行,再覆盖工作台**(2026-07-16 拆档拍板:一轮一文件、工作台只留当前轮;已闭环多轮的图注块也可顺手迁进对应轮次文件);
