...
这是 `/iterate-audit` 的快捷别名。收到后**不要请求确认**,直接读取 `.Codex/skills/iterate-audit/SKILL.md` 并从第 0 步开始执行完整一轮:

读工作台现状(`docs/功能盈利点检查.md`)→ 实施上轮待整改项(代码侧自动做,用户手动项列出)→ 无截图体检(捆绑脚本)→ **人眼走一遍转化链路**(浏览器窗格,第 2.5 步)→ 统一检查(闭环 ✅ + 新问题追加编号)→ 更新文档轮次 → 输出本轮报告。
