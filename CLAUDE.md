...
- **文件名说「装什么」,不说「是什么构造」。** `colors.ts` / `columns.ts` / `prompts.ts` 行;
  ...
  **唯一的例外是 `types.ts`**(现存 `jobs/types.ts`、`lib/pathways/types.ts`):
  形状要被同模块多个文件共用时,它有个非放不可的理由 —— 拆 `JobsTable` 时正是靠它解开了
  「服务端模块反向 import 一个 `'use client'` 组件」的倒置。只为共用的形状建,别当杂物间。
