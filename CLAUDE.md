...
- **来源真相**(2026-09-15 Frank 改判,两列分工):**渠道**列 = `origin`(被我们哪个爬虫抓进来);**来源**列 = 原始板原样显示(Job Bank 转自 indeed.com / Jobillico 的就显示那个板;原判「Job Bank 聚合的统一显示 Job Bank」撤销,那样两列 99.5% 的行一字不差);「发布」列判雇主直发;`origin` 不代表雇主真假;中介已按公司名过滤。
...
- **省份与城市的显示约定**(2026-09-11 Frank「省份统一用一种格式」「城市中文都用这种格式吧」):省份 = **主文案用界面语言全名,紧凑格/灰注用两位省码**,同一格里省份不许出现两遍(表3 省列与灰注同码实撞);城市 = **界面语言主文案 + 「英文名 省码」灰注双行形**(CityNameCell 是唯一出口;译名表外的小地方直接英文主文案,不混排)。存量不合形的按换装批清理,别顺手改。
