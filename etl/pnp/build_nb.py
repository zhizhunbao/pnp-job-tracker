...
本脚本只落**官方逐条列了 NOC 码的硬限制**(2026-02-03 起)。同页两条通告分管两个项目,**表带
`program` 字段区分**(PNP 进 08_score 资格判定;AIP 只作展示维度,08 跳过、前端判 AIP 那一行):
...
NOTICES = [
    {"key": "pnp", "program": "PNP", "must": ("expressions of interest", "NAICS 72"),
     ...
     "food": {"out": "nb-ineligible-food.json", "label": "NB 餐饮住宿不符合",
              "note": "自 2026-02-03 起,NB 不受理住宿餐饮业(NAICS 72)这些职业的省提名 EOI/ITA。" + _FOOD_NOTE}},
    {"key": "aip", "program": "AIP", "must": ("endorsement applications", "NAICS 72"),
     ...
     "food": {"out": "nb-aip-ineligible-food.json", "label": "NB AIP 餐饮住宿不受理",
              "note": "自 2026-02-03 起,NB 不受理住宿餐饮业(NAICS 72)这些职业的 AIP 背书申请。" + _FOOD_NOTE}},
]
...
def notice_of(md: str, must: tuple[str, ...]) -> str:
    """取整页里同时含这几个关键词的那条通告(同页另有一条只讲 AIP 候选池、不带 NOC 清单的,靠 NAICS 72 分开)。"""
    for b in md.split("### Notice"):
        if all(k in b for k in must):
            return b
    return ""
...
def main() -> None:
    ...
    try:
        ...
        return
    for nt in NOTICES:
        notice = notice_of(md, nt["must"])
        if not notice:
            print(f"  ✗ 没找到 {nt['key']} 通告 → NB 可能已改政策,请人工复核(保留旧表)")
            continue
        i = notice.find(SPLIT)
        ...
        for key in ("any", "food"):
            ...
            if not occs:
                ...
                continue
            table = {
                "stream": nt["stream"], "label": cfg["label"], "province": PROVINCE,
                "program": nt["program"], "type": "ineligible", "overlay": True, "note": cfg["note"],
