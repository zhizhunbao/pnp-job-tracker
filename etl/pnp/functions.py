...
from pnp.constants import (  # noqa: E402 — 段38 常量单列一块(同段35–37 先例)
    BC_NOM_THOUSANDS_SEP, K_NOMINATIONS_ISSUED_FISCAL, OUT_PE_STATS, PE_ALLOC_LABEL_TPL, PE_ALLOC_TITLE_RE, PE_FY_ASOF_TPL, PE_IIDI_NAME_TPLS,
...
def pe_table_rows_of(x: PeRowsIn) -> dict:
    ...
    while i < end:
        ...
        while j < len(x.lines) and len(nums) < x.ncols and PE_NUM_RE.match(x.lines[j]):
            nums.append(int(x.lines[j].replace(BC_NOM_THOUSANDS_SEP, "")))
            j += 1
