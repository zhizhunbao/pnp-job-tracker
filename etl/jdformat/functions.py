...
from jdformat.constants import (
    ...
    NOTE_HTTP_TPL, NOTE_LEN, NOTE_MARKS, NOTE_NO_LLM, NOTE_NO_MART, NUM_CTX, OPEN_STATUSES, OUT_FORMATTED,
    OUT_MAX_BASE, OUT_MAX_RATIO, OUT_MIN_LEN, P_MODEL, P_NUM_CTX, P_NUM_PREDICT, P_OPTIONS, P_PROMPT, P_RESPONSE,
...
def call_llm(x: LlmCallIn) -> str:
    ...
    r = x.client.post(x.cfg.base + PATH_OLLAMA_GENERATE,
                      json={P_MODEL: x.cfg.model, P_PROMPT: x.prompt, P_STREAM: False, P_THINK: False,
                            P_OPTIONS: {P_NUM_PREDICT: GEN_TOKENS, P_NUM_CTX: NUM_CTX, P_TEMPERATURE: LLM_TEMPERATURE}})
    if not r.is_success:
