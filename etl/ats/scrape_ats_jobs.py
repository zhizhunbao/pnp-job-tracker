...
2026-08-31 批F 立域搬家:自 etl/04_scrape_ats_jobs.py 迁进 ats 域改此名
(docs/design/etl分域-20260829.md §7 拍板点 8-②「ats 域 = 04 + ATS 专属清洗步」)。
搬家批 —— 抓取逻辑一字未动,只去 `__main__` 收成 run()(一域一门,门在 ats/main.py)、
补 IN_/OUT_ 常量;手动调试仍走旧 CLI 旗子(见 cli_argv)。
"""
import argparse
import json
import re
...
# ── 输入/输出全路径(同一目录进同一目录出:一司一档,就地富化)──────────
IN_COMPANIES = paths.COMPANIES    # 逐司读 careers.json(processed/<region>/companies/<slug>/)
OUT_COMPANIES = paths.COMPANIES   # 就地写回 jobs.json + jobs/<职位>.md
...
def main(argv: list[str]) -> None:
    ...
    ap.parse_args(argv)
    region_dir = IN_COMPANIES  # paths.COMPANIES 已含地域(processed/<region>/companies)
...
ONLY_FLAG = "--only"          # 门(main.py)自己的旗子,不是本件的,摘掉再交给 argparse


def cli_argv() -> list[str]:
    """进程 argv 里本件自己的旗子(门的 `--only <步>` 摘掉);役册链上这一步本就不带实参。"""
    argv = list(sys.argv[1:])
    if ONLY_FLAG in argv:
        i = argv.index(ONLY_FLAG)
        del argv[i:i + 2]
    return argv


def run() -> None:
    """本域步骤入口:逐司读 careers.json → 调各家 ATS 公开 JSON → 写回 jobs.json + jobs/*.md。"""
    main(cli_argv())
