"""
jobboom 域函数 —— 四段与 constants.py / scheme.py 同名同序镜像:站点地图枚举(剔 Job Bank 转载)→ 详情原文抓取
→ 详情解析 → postings 仓。顶层只有 function;常量归 constants,形状归 scheme。与 jobillico 域同形不共码(一站一域)。
...
def urls_by_id(locs: list) -> UrlsOut:
    """帖子 URL 清单 → 帖号 → URL(只认详情 URL 形;雇主段 = job-bank 的是 Job Bank 转载,剔并计数)。"""
    urls: dict = {}
    syndicated = 0
    for loc in locs:
        ...
        if m is None:
            continue
        if m.group(1) == SYNDICATED_EMPLOYER:
            syndicated += 1
            continue
        urls[m.group(2)] = loc
    return UrlsOut(urls=urls, syndicated=syndicated)


def lang_of(url: str) -> str:
    """详情 URL 的语言(只走英文站点地图:认得出 URL 形即 en,认不出给空串)。"""
...
def scrape_jobboom_details() -> None:
...
def build_jobboom_postings() -> None:
