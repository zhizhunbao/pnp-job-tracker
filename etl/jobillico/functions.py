...
from jobillico import DETAILS_PER_RUN
from jobillico.constants import (
    ...
    OUT_URLS, PERCENT, PRINT_DETAIL_DONE_TPL, PRINT_DETAIL_HEAD_TPL, PRINT_DETAIL_TICK_TPL,
    PRINT_PARSE_DONE_TPL, PRINT_SITEMAP_TPL, PRINT_STORE_DONE_TPL, PRINT_URLS_DONE_TPL, RATE_FLOOR_S,
    SALARY_RANGE_TPL, SALARY_TPL, SALARY_UNIT_WORD, SECONDS_FMT, SITEMAP_INDEX_URL, SITEMAP_JOBS_RE,
    SLUG_CRAWL, SOURCE_LABEL, SPACE, TERM_OF_TYPE, UTC_Z, WS_RE,
)
from jobillico.constants import (
...
def scrape_jobillico_sitemap() -> None:
    """本域步骤入口:索引 → 职位子图 → 帖号 → URL 表(同帖号英法两版取英文)。"""
    locs: list = []
    with make_client(CLIENT_TIMEOUT_S) as raw_client:
        client = cast(HttpClientLike, raw_client)
        for child in collect_sitemap_urls(SitemapIn(client=client, url=SITEMAP_INDEX_URL)):
            if SITEMAP_JOBS_RE.search(child) is None:
                continue
            found = collect_sitemap_urls(SitemapIn(client=client, url=child))
            say(PRINT_SITEMAP_TPL.format(url=child, n=len(found)))
            locs.extend(found)
    urls = urls_by_id(locs)
    en = 0
    for url in urls.values():
        if lang_of(url) == LANG_EN:
            en += 1
    ...
    say(PRINT_URLS_DONE_TPL.format(ids=len(urls), en=en, fr=len(urls) - en, out=OUT_URLS))
...
def urls_by_id(locs: list) -> dict:
    """帖子 URL 清单 → 帖号 → URL(只认详情 URL 形;同帖号两版英文优先)。"""
    out: dict = {}
    for loc in locs:
        m = JOB_URL_RE.match(loc)
        if m is None:
            continue
        pid = m.group(2)
        if pid in out and lang_of(out[pid]) == LANG_EN:
            continue
        out[pid] = loc
    return out
...
def scrape_jobillico_details() -> None:
...
def job_posting_of(html: str) -> dict | None:
    """页里的 ld+json 块中取 JobPosting 那块;没有给 None。"""
...
def to_job_fact(x: LdPostingIn) -> JobFact:
    ...
    return JobFact(
        posting_id=x.posting_id, url=x.url, lang=x.lang,
...
def build_jobillico_postings() -> None:
    """本域步骤入口:事实表 × 枚举表 → 当前态 postings 仓(不在站点地图 / 过截止日 / 无标题的剔)。
