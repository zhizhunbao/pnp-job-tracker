...
from mart.constants import (
    ...
    K_DESCRIPTION, K_DISTRICT, K_DRAWS, K_ELIGIBILITY_FLAG, K_ELIGIBILITY_QUOTE, K_ELIGIBLE, K_SRC_VALID_THROUGH,
    K_VALID_THROUGH,
    ...
    SK_CAPPED_METRICS, SK_PROC_LABEL_TPL, SLASH, SLUG_DAILY_TOP, SLUG_APOSTROPHE_RE, SLUG_DASH, SLUG_FALLBACK,
...
def slugify(s: str) -> str:
    """任意文本 → URL 段(小写、删撇号、非字母数字压连字符、去首尾连字符、≤60 字;空则兜底 'company')。"""
    cut = SLUG_RE.sub(SLUG_DASH, SLUG_APOSTROPHE_RE.sub("", (s or "").lower())).strip(SLUG_DASH)[:SLUG_MAX]
    if cut:
        return cut
...
def fill_brief(x: CompanyExtraIn) -> None:
    ...
    if br.get(K_BRIEF_KO) and BRIEF_KO_SCRIPT_RE.search(br[K_BRIEF_KO]):
...
def macro_pr_rows() -> list:
    """PR 登陆数按年 → prAll(全部类别)/ prPnp(省提名)+ 大组 / 中类 / 细类三级族行。

    2026-09-10 Frank「其他的项的 pr 人数是不是也需要列一下」:把脉页每省小表补
    「其中家庭团聚 / 其中难民 / 其他类」。raw 里没有的键不出行(官方那版表没出那一组)。
    2026-09-10 同日再发中类五键与细类十九键(MACRO_KEYS_PR):键表一扩,本函数一字不改 ——
    每一键都是官方原行、同一套解法。
