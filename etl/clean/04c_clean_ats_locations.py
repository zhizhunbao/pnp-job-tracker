...
def normalize_jobbank(prov: str, city: str, addr: str) -> dict:
    ...
    if city_c.lower().startswith(NON_CITY_PREFIXES):  # 占位词不是城市 → 留空
        city_c = ""
...
def main() -> None:
    ...
    if IN_JOBBANK_FILE.exists():
        ...
        # 省份兜底:Job Bank 偶尔漏填某帖的省。若同一 city_raw 在别处出现且全数据集只映射到唯一省,
        # 用那个省补空(同名同源,非臆测);名字跨省(London/Windsor…)则留空,不猜。
        from collections import defaultdict
        city_provs = defaultdict(set)
        for j in posts:
            key, prov = (j.get("city_raw") or "").strip().lower(), (j.get("province") or "").strip()
            if key and prov:
                city_provs[key].add(prov)
        backfilled = 0
        for j in posts:
            if (j.get("province") or "").strip():
                continue
            cands = city_provs.get((j.get("city_raw") or "").strip().lower())
            if cands and len(cands) == 1:
                j["province"] = next(iter(cands))
                backfilled += 1
        if backfilled:
