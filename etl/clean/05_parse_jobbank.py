...
读哪份:`raw/httpx/jobbank/<最新日期>/` 目录(日期直接挂源下,与 details/ 平级;按目录名取 max),
...
IN_SNAP_ROOT = _paths.RAW_HTTPX_JOBBANK                       # 列表 HTML 快照:<日期>/ 直接挂源下
...
def latest_snapshot_dir() -> Path | None:
    if not IN_SNAP_ROOT.exists():
        return None
    days = sorted((d for d in IN_SNAP_ROOT.iterdir() if d.is_dir() and _DATE_RE.match(d.name)), key=lambda d: d.name)
