...
Usage:  uv run python etl/jobbank/main.py --only companies

2026-08-31 批F 立域搬家:自 etl/06_build_jobbank_companies.py 迁进 jobbank 域改此名
(docs/design/etl分域-20260829.md §7 拍板点 8「build 应该每个域自己管自己的」——
公司档是 jobbank 的 build,谁的数据谁管)。搬家批 —— 构建逻辑一字未动,只去 `__main__`
收成 run()、sys.path 交给门(一域一门,§4 方言律 ⑦)。
**手动件**:全仓零调度消费者(旧 sources/jobbank 与 sources/build 役册都不含它),
只进本域 TOOLS 供手动点名,不进 SCHEDULED。
"""
import json
import re
...
def run() -> None:
    """本域步骤入口:扁平 postings.json → 分省/市/雇主的公司目录(确定性、不联网)。"""
    main()
