...
ENV_REPARSE_IDS = "REPARSE_IDS"
"""环境变量名:一份帖号名单文件的路径(一行一个帖号)—— 只强制重解析名单里的帖(2026-09-19:REPARSE=1 全量过 16 万份快照,
绑定卷上实测两小时才 1.9 万份、要十几个小时,期间占着仓锁把每小时的汇装灌库堵死;正文换行修复只涉及 7,435 条压平帖,定点重解析几分钟)。"""
...
SANITY_WAGE_NATIONAL = "NAT"
"""护栏查中位时的全国兜底键(wages.json 全国行的真实键)。原 clean/05d 起就误写 "CA",
这一支从未取到值、护栏一直落 ABS_FLOOR 兜底;2026-08-31 批J 溶解只搬不改留了台账,
同日 Frank 拍板修正。探针(12.5 万帖):候选 13 帖全走此兜底、全部翻面 —— 清一色
「intern × NOC 31102 × 年薪 60,500」蹭过 6 万硬底的原型案例,对全国中位 232,227 即失配。"""
