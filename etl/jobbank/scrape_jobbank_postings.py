...
2026-08-31 批F 立域搬家:自 etl/05_scrape_jobbank.py 迁进 jobbank 域改此名
(docs/design/etl分域-20260829.md §7 拍板点 8「jobbank 肯定是需要一个域的」)。
搬家批 —— 抓取逻辑一字未动,只去 `__main__` 收成 run()(一域一门,门在 jobbank/main.py),
役册链上实参内置进 chain_argv();手动调试仍走旧 CLI 旗子(见 cli_argv)。
"""
import argparse
import csv
import json
...
# ── 役册链上实参(2026-08-31 批F 自 sources/jobbank META.steps 逐字搬入)────────
...
def chain_argv() -> list[str]:
    """役册链上那一步的实参:全职业 · 全省 · 最近 SINCE_DAYS 天 · 失控保险 400 页。"""
    return ["--all-occupations", "--prov", "ALL",
            "--since-days", SINCE_DAYS, "--max-pages", CHAIN_MAX_PAGES]


def cli_argv() -> list[str]:
    """进程 argv 里本件自己的旗子(门的 `--only <步>` 摘掉);一个旗子都没有 → 用链上实参。"""
...
def run() -> None:
    """本域步骤入口:抓 listing 原始 HTML 快照。带旗子=旧 CLI 行为,不带=役册链上那一步。"""
    main(cli_argv())
