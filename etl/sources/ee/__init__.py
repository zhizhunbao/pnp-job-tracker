...
前端分页)→ `build_ircc_ee_categories.py` 直取解析 9 类 ~94 NOC → `raw/ee/federal-categories.json`;
`build_ircc_ee_draws.py` 取 IRCC 开放 JSON → `raw/ee/draws.json`(每类别最近抽选 CRS/日期)。
...
META = {
    ...
    "steps": [
        ["python", "etl/ee/build_ircc_ee_categories.py"],
        ["python", "etl/ee/build_ircc_ee_draws.py"],
        ["python", "etl/lmia/build_esdc_lmia_employers.py"],   # E6-02:ESDC LMIA 季度数据(月检查,已缓存季度不重下;需镜像含 openpyxl)
    ],
}
