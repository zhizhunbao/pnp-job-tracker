// warn 棘轮(Frank 2026-08-21「先写闸门」):已重构十五域的 eslint warning 总数只许降不许升。
//
...
const SCOPE = [
  'src/lib/consult', 'src/lib/db', 'src/lib/employers', 'src/lib/jobs', 'src/lib/pathways', 'src/lib/plan', 'src/lib/stats', 'src/lib/resume', 'src/lib/ruling', 'src/lib/gauge', 'src/lib/points',
...
if (warnings > baseline.warnings) {
  console.error(`[ratchet] 十五域 warning ${warnings} 条,高于基线 ${baseline.warnings} —— 棘轮只许降不许升,把新增的清掉再推。`)
  process.exit(1)
}
if (warnings < baseline.warnings) {
  writeFileSync(BASELINE_PATH, JSON.stringify({ scope: baseline.scope, warnings }, null, 2) + '\n')
  console.log(`[ratchet] 十五域 warning ${warnings} 条,低于基线 ${baseline.warnings} —— 基线已收紧,记得 git add .githooks/lint-baseline.json。`)
  process.exit(0)
}
console.log(`[ratchet] 十五域 warning ${warnings} 条,与基线持平。`)
